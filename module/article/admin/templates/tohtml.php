...
<?php if(is_array($_CAT)) foreach($_CAT AS $cat) {
	if($cat[cattype]){
		$p->set_type("path");
...
?>
  <tr align=center>
    ...
    <td class="tablerow"><a href='?mod=<?=$mod?>&file=<?=$file?>&action=category&catid=<?=$cat[catid]?>&channelid=<?=$channelid?>'>生成栏目列表</a> | <a href='?mod=<?=$mod?>&file=<?=$file?>&action=catarticle&catid=<?=$cat[catid]?>&channelid=<?=$channelid?>'>生成文章</a> | <a href='?mod=<?=$mod?>&file=<?=$file?>&action=createcatdir&catid=<?=$cat[catid]?>&channelid=<?=$channelid?>'>生成栏目目录</a></td>
  </tr>
<?php
	}
} ?>
  ...
  <tr>
	...
	<input type="submit" name="submit2" value="生成文章" onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=catarticle&channelid=<?=$channelid?>'">&nbsp;&nbsp;
...
<tr align="center">
<td width="10%" class="tablerowhighlight">文章起始ID</td>
<td width="45%" class="tablerowhighlight">文章结束ID</td>
<td width="15%" class="tablerowhighlight">每轮生成文章数</td>
<td width="30%" class="tablerowhighlight">管理操作</td>
</tr>
<form method="post" name="myform2" action="?mod=<?=$mod?>&file=<?=$file?>&action=article&channelid=<?=$channelid?>">
  <tr align=center>
    ...
    <td class="tablerow"><input name='submit' type='submit' value=' 生成html '>&nbsp;&nbsp;<a href='?mod=<?=$mod?>&file=article&action=manage&channelid=<?=$channelid?>'><b>进入文章管理&gt;&gt;</b></a></td>
  </tr>
</form>
</table>
<table cellpadding="0" cellspacing="0" border="0" width="100%" height="10">
...
<? if(is_array($specials)) foreach($specials AS $special) { ?>
  <tr>
    <td align="center" class="tablerow"><input type="checkbox" name="specialid[]"  id="specialid[]" value="<?=$special[specialid]?>"></td>
