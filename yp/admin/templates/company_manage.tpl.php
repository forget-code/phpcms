...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage">企业管理首页</a> &gt;&gt; 管理企业</td>
...
<td><a href="<?=$MOD['linkurl']?><?=$TRADE[$company['tradeid']]['linkurl']?>" target="_blank"><?=$TRADE[$company['tradeid']]['tradename']?></a></td>
<td><a href="<?=$company['linkurl']?>" target="_blank"><?=$company['companyname']?></a> <?php if($company['elite']) echo '<font color="green">荐</font>'; ?> <?php if($company['vip'] && $company['vip']>$PHP_TIME) echo '<font color="red">VIP</font>'; elseif($company['vip'] && $company['vip']<$PHP_TIME) echo '<font color="#5F5F5F">VIP</font>';?></td>
