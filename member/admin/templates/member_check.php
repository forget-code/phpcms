...
<td class=forumrow><a href="<?=PHPCMS_PATH?>member.php?action=show&username=<?=urlencode($member[username])?>" target="_blank"><?=$member[username]?></a></td>
<td class=forumrow><?=$member[truename]?></td>
<td class=forumrow><?=$member[gender]?></td>
<td class=forumrow><?=$member[province]?>-<?=$member[city]?></td>
<td class=forumrow><?=$member[email]?></td>
<td class=forumrow><?=$member[regtime]?></td>
<td class=forumrow><?=$member[regip]?></td>
...
<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td>
<input type="submit" name="submit" value="批量批准" onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=pass&val=1'">&nbsp;&nbsp;
...
<td align=center width="10%" class="tablerowhighlight">注册时间</td>
<td width="15%" class="tablerow">
<select name='regdate'>
<option value='0'>不限</option>
<option value='1' <?php if($regdate==1){ ?>selected <?php } ?>>今日注册</option>
<option value='2' <?php if($regdate==2){ ?>selected <?php } ?>>昨日注册</option>
<option value='7' <?php if($regdate==7){ ?>selected <?php } ?>>一周内注册</option>
<option value='14' <?php if($regdate==14){ ?>selected <?php } ?>>两周内注册</option>
<option value='30' <?php if($regdate==30){ ?>selected <?php } ?>>一月内注册</option>
</select>
</td>
...
<td align=center width="10%" class="tablerowhighlight">性别</td>
<td width="15%" class="tablerow">
<select name='gender'>
