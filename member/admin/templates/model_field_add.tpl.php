...
<form action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&modelid=<?=$modelid?>" method="post" name="myform">
  <caption>添加字段</caption>
	<tr> 
      ...
      <td> <input size="20" name="info[field]" id="field" size="20" require="true" datatype="limit|ajax" min="1" max="20" url="?mod=<?=$mod?>&file=<?=$file?>&action=checkfield&modelid=<?=$modelid?>" msg="字符须为1到10位|"><font color="red">*</font></td>
    </tr>
	<tr> 
	...
	<tr> 
      <th><strong>字段提示</strong><br />显示在字段别名下方作为表单输入提示</td>
...
?>
	...
	<tr> 
      ...
      <td><input type="radio" name="info[islist]" value="1" checked /> 是 <input type="radio" name="info[islist]" value="0"   /> 否</td>
    </tr>
	<tr> 
