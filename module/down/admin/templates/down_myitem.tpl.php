...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=main&channelid=<?=$channelid?>">下载首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage&job=myitem&catid=<?=$catid?>&channelid=<?=$channelid?>">我添加的下载</a> &gt;&gt; <?=$cat_pos?> </td>
...
<?php if($CHA['ishtml']){ ?><input name='submit1' type='submit' value='生成下载' onClick="document.myform.action='?mod=<?=$mod?>&file=tohtml&action=downtohtml&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp; <?php } ?>
        <input name='submit2' type='submit' value='推荐下载' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=elite&value=1&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp;
