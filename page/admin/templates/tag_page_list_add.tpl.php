  ...
  <form name="myform" method="get" action="?" onsubmit="return doCheck();">
   ...
   <input type="hidden" name="tag_config[func]" value="<?=$function?>">
    <tr> 
      ...
      <td  class="tablerow">
	  <input name="tagname" id="tagname" type="text" size="20" value="<?=$tagname?>"> <input type="button" value=" 检查是否已经存在 " onclick="Dialog('?mod=<?=$mod?>&file=<?=$file?>&action=checkname&keyid=<?=$keyid?>&tagname='+$('tagname').value+'','','300','40','no')"> <br/>
	  </td>
    </tr>
    <tr> 
    ...
    <tr> 
      ...
      <td  class="tablerow"><input name="tag_config[num]" id="num" type="text" size="15" value="0"> 填0表示调用所选模块所有单网页
	  </td>
    </tr>

    <tr> 
      <td class="tablerow"><b>所属模块</b></td>
      <td  class="tablerow"><input name="tag_config[keyid]" id="keyid_select" type="text" size="15" value="<?=$keyid?>"> 
<?=keyid_select('', '所属模块', $keyid, 'onchange="$(\'keyid_select\').value=this.value"')?>
    ...
    <tr> 
      ...
      <td  class="tablerow">
	<?=showtpl($mod,'tag_page_list','tag_config[templateid]')?>
