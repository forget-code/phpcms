...
<table cellpadding="2" cellspacing="1" class="tableborder">
  ...
  <tr>
    ...
    <tr <?php if(!$MOD['author_show']){ ?>style="display:none"<?php } ?>> 
      ...
      <td class="tablerow"><input name="article[author]" type="text" id="author" size="40" maxlength="30">
	  <?=$author_select?>
    ...
    <tr <?php if(!$MOD['copyfrom_show']){ ?>style="display:none"<?php } ?>> 
      ...
      <td class="tablerow"><input name="article[copyfrom]" type="text" id="copyfrom" size="40">
		<?=$copyfrom_select?>
    ...
    <tr> 
	  ...
	  <br/><br/>
	  <input name="add_introduce" type="checkbox"  value="1" <?php if($MOD['add_introduce']){ ?>checked<?php } ?>>是否截取内容<br><input type="text" name="introcude_length" value="<?=$MOD['introcude_length']?>" size="3">字符至内容摘要
	  <br/><br/>
	  <input name="auto_thumb" type="checkbox"  value="1" <?php if($MOD['auto_thumb']){ ?>checked<?php } ?>>是否获取内容第<br><input type="text" name="auto_thumb_no" value="1" size="1">张图片作为标题图片
	  ...
	  <tr>
	  <td valign="top">
        <textarea name="article[content]" id="content" cols="100" rows="25"></textarea> <?=editor("content", $MOD['editor_mode'],$MOD['editor_width'],$MOD['editor_height'])?>
    </td>
</tr>
</table>
...
<input name="article[status]" type="radio" value="3" <?php if($_grade<4){?>checked<?php }else{ ?>disabled<?php } ?>> 已通过&nbsp;
<input name="article[status]" type="radio" value="1" <?php if($_grade==4){?>checked<?php } ?>> 待审核&nbsp;
...
<tr>
<td width="64">完整标题</td>
<td class="tablerow"><input name="article[titleintact]" type="text" id="titleintact" size="80" maxlength="255"></td>
</tr>
<tr>
<td width="64">副 标 题</td>
<td class="tablerow"><input name="article[subheading]" type="text" id="subheading" size="80" maxlength="255"></td>
</tr>
<tr>
<td class="tablerow">&nbsp;</td>
<td class="tablerow"><input name="article[showcommentlink]" type="checkbox" id="showcommentlink" value="1">显示文章列表时在标题旁显示评论链接</td>
</tr>
</table>
    </td>
    </tr>
    ...
    <tr> 
      ...
      <td class="tablerow"><input type="radio" name="article[ishtml]" value="1" <?php if($CAT['ishtml']==1) {?>checked <?php } ?>  onclick="$('htmlrule').style.display='';$('htmlprefix').style.display='';$('htmldir').style.display='';$('phprule').style.display='none';"> 是 <input type="radio" name="article[ishtml]" value="0" <?php if($CAT['ishtml']==0) {?>checked <?php } ?> onclick="$('htmlrule').style.display='none';$('htmldir').style.display='none';$('htmlprefix').style.display='none';$('phprule').style.display='';"> 否</td>
		...
		<tr id="htmldir" style="display:<?php if($CAT['ishtml']==0) {?>none<?php }?>"> 
		  ...
		  <td class="tablerow"><input type="text" name="article[htmldir]" value="<?=$CAT['item_htmldir']?>" id="htmldir" ></td>
		</tr>
		<tr id="htmlprefix" style="display:<?php if($CAT['ishtml']==0) {?>none<?php }?>"> 
		  <td class="tablerow">html文件名前缀</td>
		  <td class="tablerow"><input type="text" name="article[prefix]" id="prefix" value="<?=$CAT['item_prefix']?>"></td>
		</tr>
