...
<caption>网址采集</caption>
    ...
    <td width="*" class='tablerow'><?=$site_select?>&nbsp;&nbsp;<a href="?mod=<?=$mod?>&file=sitemgr&action=add"><font color="blue">添加站点</font></a></td>
      </tr>
          <tr>
    <td width="150"  class='tablerow'><strong>任务名称:</strong>      </td>
    <td width="*" class='tablerow'><input name='job[JobName]' type='text' id='jobJobName' size='20'>&nbsp;<font color='#FF0000'>*</font></td>
    </tr>
	<tr>
	<td width="150" class='tablerow'><strong>发布栏目:</strong></td>
	<td width="*" class='tablerow'><input name="job[CatId]" id="catid" type="hidden" size="10" value="0"/>
	<?=form::select_category('phpcms', 0, 'parentid', 'parentid', '无（作为一级栏目）', $catid,$cat_js)?>&nbsp;<font color="Red">*</font></td>
	</tr>
	<tr>
    <td width="150"  class='tablerow'><strong>简单描述:</strong>      </td>
    <td width="*" class='tablerow'><textarea name="job[JobDescription]" cols="80" rows="3" id="jobJobDescription"></textarea></td>
   </tr>
  ...
  <tr>
    <td  class='tablerow'><strong>开始采集地址</strong>:<br>一行一个</td>
    <td class='tablerow'><input type="button" name="add" value="批量添加多页" onclick="document.all.addresssimilar.style.display=''"><br><textarea name="StartUrl" id="StartUrl" cols="80" rows="6" ></textarea></td>
  </tr>
  <tr>
    ...
    <td class='tablerow'><table width="98%"  border="0">
        <tr>
          <td width="628">从
            <textarea name="job[ListUrlStart]" cols="35" rows="3" id="textarea2"></textarea>
到
<textarea name="job[ListUrlEnd]" cols="35" rows="3" id="textarea3"></textarea></td>
  ...
  <tr>
    ...
    <td class='tablerow'>&nbsp;网址中<b>必须</b>包含
        <input name='job[ContentPageMust]' type='text' id='jobListPageMust' size='30'>
&nbsp;&nbsp;&nbsp;网址中<b>不得</b>包含
      <input name='job[ContentPageForbid]' type='text' id='jobContentPageForbid' size='30'></td>
  </tr>
  <tr>
    <td  class='tablerow'><strong>缩略图采集规则:</strong><br />建议不填写<br />不填写时程序抓取第一个图片为缩略图</td>
    <td class='tablerow'><textarea name='job[SpiderRule]' id='SpiderRule' cols="60" rows="3"></textarea>&nbsp;通配符: <a href="javascript:AddOnPos(document.myform.SpiderRule,'(*)');"  style="color:#3300FF;">(*)</a>
  ...
  <tr>
    ...
    <td class='tablerow'>使用已有的COOKIE:
        <input name='job[Cookie]' type='text' id='job[Cookie]' size='66'></td>
  </tr>
</table>
...
<caption>内容规则</caption>
  ...
  $i=0;
  $ph=PHPCMS_PATH;
  if(!isset($labelkeys) && is_array($labels))
  {
...
echo <<<EOT
  <tr bgcolor="#CCFFCC">
    <td colspan="2"  style="cursor:hand" onClick="ShowLabel('{$label["name"]}');"><table width="100%"  border="0">
        <tr>
		  <td width="78%" bgcolor="#6699FF"><img src="spider/images/{$label["imgstatus"]}.gif" width="18" height="18" id="{$label["name"]}img">
		  <strong  id='rule{$i}name'>{$label["name"]}</strong>[点击打开/隐藏标签] </td>
          <td width="22%" bgcolor="#6699FF" align="right">No.{$i}&nbsp;&nbsp;&nbsp;发布字段
		  <span id="publish" name="{$label['name']}"></span>
		  </td>
        </tr>
      </table></td>
    </tr>
...
EOT;
	...
	else if(isset($rule) && is_array($rule['LabelName']))
...
<tr bgcolor="#CCFFCC">
	...
	<table width="100%"  border="0">
        <tr>
          <td width="70%" bgcolor="#6699FF"  style="cursor:hand" onClick="ShowLabel('${"label_".$labelkeys[$k]}');"><img src="spider/images/open.gif" width="18" height="18" id="${"label_".$labelkeys[$k]}img">
		  <strong  id='rule$labelkeys[$k]name'>${"label_".$labelkeys[$k]}</strong>: [点击打开/隐藏标签] </td>
          <td width="22%"  bgcolor="#6699FF" align="right">No.{$labelkeys[$k]}&nbsp;&nbsp;&nbsp;发布字段<span id="publish" name="${"label_".$labelkeys[$k]}"></span>
		  </td>
        </tr>
      </table></td>
    </tr>
  ...
  <tr>
    ...
    <td valign="middle" class='tablerow'><table width="98%"  border="0">
        <tr>
          ...
          <td width="223"><textarea name="job[DividePageStart]" cols="26" rows="4" id="jobDividePageStart"><?if(isset($labelkeys)) echo $rule['DividePageStart']; ?></textarea>            </td>
          <td width="13">到</td>
          <td width="223"><textarea name="job[DividePageEnd]" cols="26" rows="4" id="jobDividePageEnd"><?if(isset($labelkeys)) echo $rule['DividePageEnd']; ?></textarea></td>
          <td width="220"><input type='radio' name='job[DividePageStyle]'  value='0' <?if(isset($labelkeys)&&$rule['DividePageStyle']=="0") echo " checked"; ?>>
全部列出模式
  <br>
  <input type='radio' name='job[DividePageStyle]' value='1'  <?if(isset($labelkeys)&&$rule['DividePageStyle']=="1") echo " checked"; ?>>上下页模式</td>
<td width="150">
...
通配符:<a href="javascript:AddOnPos(document.myform.jobDividePageEnd,'(*)');" style="color:#3300FF;">(*)</a>
	  ...
	  分页合并代码：<textarea name="job[DividePageUnion]" cols="26" rows="4" id="jobDividePageUnion">[page]</textarea>
	  </td>
    </tr>
    </tr> 
    <tr >
    <td width="192"  class='tablerow'><strong>发布自动分页大小:</strong>      </td>
    <td width="*" class='tablerow'><input type="text" name="job[AutoPageSize]" cols="26" rows="4" id="jobAutoPageSize" value="0">此处填写自动分页时单页字符长度，0表示不分页。</td>
    </tr>      
  <tr>
</table>
...
<caption>高级设置</caption>
  <tr>
    ...
    <td class='tablerow'>
      <input type='radio' name='job[SiteEncode]' value='0' checked>
      GB2312&nbsp;&nbsp;&nbsp;&nbsp;
      <input type='radio' name='job[SiteEncode]' value='1'>
      UTF8&nbsp;&nbsp;&nbsp;&nbsp;
      <input type='radio' name='job[SiteEncode]' value='2'>
      BIG5 </td>
  </tr>
  <tr>
    <td class='tablerow'><strong>内容页编码设置</strong></td>
    <td class='tablerow'>
      <input type='radio' name='job[SourceEncode]' value='0' checked>
      GB2312&nbsp;&nbsp;&nbsp;&nbsp;
      <input type='radio' name='job[SourceEncode]' value='1'>
      UTF8&nbsp;&nbsp;&nbsp;&nbsp;
      <input type='radio' name='job[SourceEncode]' value='2'>
      BIG5 </td>
  </tr>

  <tr>
    <td  class='tablerow'><strong>下载图片</strong></td>
    <td class='tablerow'><input type="checkbox" name="job[DownImg]" id="jobDownImg" value="0" checked="checked">
      下载采集内容中的图片资源，自动保存到本地服务器</td>
  </tr>
  <tr>
    <td  class='tablerow'><strong>下载Flash</strong></td>
    <td class='tablerow'><input type="checkbox" name="job[DownSwf]" id="jobDownSwf" value="0">
      下载采集内容中的Flash资源 ，自动保存到本地服务器 </td>
  </tr>
    <tr>
    <td  class='tablerow'><strong>下载其他文件</strong></td>
    <td class='tablerow'>是否下载<input type="checkbox" name="job[DownOther]" id="jobDownOther">  文件后缀形式:<input type="text" name="job[OtherFileType]" id="jobOtherFileType" value="mp3|rar|zip">
      <font color="#AAAAAA">只适用于下载显示真实地址的文件，文件不宜过大</font></td>
  </tr>
 <tr>
    <td  class='tablerow'><strong>多线程设置</strong></td>
    <td class='tablerow'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    线程数:<input name='job[ThreadNum]' type='text' id='jobThreadNum' size='8' value='2' maxlength="1"><br />
    每个线程同时请求连接数:<input name='job[ThreadRequest]' type='text' id='jobThreadRequest' size='8'  value='2' maxlength="1"><br />
    线程间隔时间(单位为秒):<input name='job[ThreadSleep]' type='text' id='jobThreadSleep' size='8' value='0' maxlength="2"><br /></td>
  </tr>
   <tr>
    <td  class='tablerow'><strong>超时时间</strong></td>
    <td class='tablerow'><input name='job[TimeOut]' type='text' id='jobTimeOut' size='6'   value='120'>秒 (0为无限大)</td>
  </tr>
  <tr>
    <td>
</table>
...
function CheckForm()
{
	if(document.myform.jobJobName.value=='')
	{
		ShowTabs(0);
		alert('请添加任务名称！');
	...
	if(document.myform.StartUrl.value==''||document.myform.StartUrl.value=='http://')
	{
		ShowTabs(0);
		alert('请填写开始采集地址！');
		document.myform.StartUrl.focus();
		return false;
	}
	...
	if(document.getElementById("catid").value=='0')
	{
		ShowTabs(0);
		alert('请选择要发布的栏目！');
		document.myform.parentid.focus();
		return false;
	}else
	{
		var obj = $("span") || '';
...
function AddLabel(labelname,lnum)
{
	...
	document.all.userlabel.innerHTML+="<div id=\"zhu"+labelnum+"\" style=\"display:block\"><table cellpadding='0' cellspacing='0' border='0' width='96%' class='table_form' align='center' >"+labelhtml+"</table></div>";
	document.all.userlabel.innerHTML+="<div id=\"fu"+labelnum+"\"  style=\"display:block\"><table width='100%' align='center'><tr ><td background='<?=PHPCMS_PATH?>/images/spacer.gif'></td></tr></table></div>";
	//自动载入发布字段模块
	setModelSelected();
