...
switch($action)
{
	case 'add':
		if($dosubmit)
		{
			...
			if($category['catdir']=='web') showmessage($LANG['choose_othor_dir']);
			...
			if(!$category['islink'])
			{
				$r = $db->get_one("select catid from ".TABLE_CATEGORY." where catdir='$category[catdir]' and module='$module' ");
		...
		else
	    {
		    ...
		    $skinid = showskin('setting[skinid]','yp');
            $defaultitemtemplate = showtpl($module,'content','setting[defaultitemtemplate]');
		    $defaultitemskin = showskin('setting[defaultitemskin]','yp');
			$page_select = page_select(0,"onchange='document.myform.linkurl.value=this.value'");
			$cat_html_urlrule = yp_urlrule_select('category[cat_html_urlruleid]','html','cat');
			$cat_php_urlrule = yp_urlrule_select('category[cat_php_urlruleid]','php','cat');
	...
	case 'edit':
		...
		else
	    {
		    ...
		    $cat_html_urlrule = yp_urlrule_select('category[cat_html_urlruleid]','html','cat',$cat_html_urlruleid);
			$item_html_urlrule = yp_urlrule_select('category[item_html_urlruleid]','html','item',$item_html_urlruleid);
			$cat_php_urlrule = yp_urlrule_select('category[cat_php_urlruleid]','php','cat',$cat_php_urlruleid);
			$item_php_urlrule = yp_urlrule_select('category[item_php_urlruleid]','php','item',$item_php_urlruleid);
		    include admintpl('category_edit');
		}
		break;

     case 'recycle':
		 $forward = '?mod=yp&file=category&action=manage';
		 $module = $mod;
		 $catid = intval($catid);
		 $r=$db->get_one("SELECT arrchildid FROM ".TABLE_CATEGORY." WHERE catid=$catid");
		 $arrchildid = $r['arrchildid'];
		 $db->query("UPDATE ".TABLE_YP_PRODUCT." SET status=-1 WHERE catid IN ($arrchildid)");
	...
	case 'updatecache':
		...
		showmessage($LANG['category_cache_update_success'], $forward,40000);
		break;
	...
	case 'manage':
		...
		if(is_array($list))
	    {
			...
			$str = "<tr align='center' align='center' onmouseout=this.style.backgroundColor='#F1F3F5' onmouseover=this.style.backgroundColor='#BFDFFF' bgColor='#F1F3F5'>
						...
						<td>\$catdir</td>
						<td></td>
						<td><a href='?mod=\$mod&file=\$file&action=add&catid=\$id'>".$LANG['add_child_category']."</a> | <a href='?mod=\$mod&file=\$file&action=edit&catid=\$id&parentid=\$parentid'>".$LANG['edit']."</a> | <a href=javascript:confirmurl('?mod=\$mod&file=\$file&action=recycle&catid=\$id','".$LANG['confirm_clear_category_info']."') >".$LANG['clear']."</a> | <a href=javascript:confirmurl('?mod=\$mod&file=\$file&action=delete&catid=\$id','".$LANG['confirm_delete_category']."')>".$LANG['delete']."</a></td></tr>";
			$tree->tree($categorys);
