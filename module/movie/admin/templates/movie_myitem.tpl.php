...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=main&channelid=<?=$channelid?>">影片首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage&job=myitem&catid=<?=$catid?>&channelid=<?=$channelid?>">我添加的影片</a> &gt;&gt; <?=$cat_pos?> </td>
...
<td><?=$movie['hits']?></td>
<td>
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=edit&channelid=<?=$channelid?>&movieid=<?=$movie['movieid']?>&catid=<?=$movie['catid']?>&referer=<?=$referer?>" title="编辑影片">编辑</a> |
...
<?php if($CHA['ishtml']){ ?><input name='submit1' type='submit' value='生成影片' onClick="document.myform.action='?mod=<?=$mod?>&file=tohtml&action=movietohtml&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp; <?php } ?>
        <input name='submit2' type='submit' value='推荐影片' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=elite&value=1&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp;
		...
		<input name='submit7' type='submit' value='删除影片'		onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=-1&channelid=<?=$channelid?>&referer=<?=$referer?>'">&nbsp;
        <input name='submit8' type='submit' value='移动影片' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=move&movetype=1&channelid=<?=$channelid?>'"></td>
  </tr>
</table>
