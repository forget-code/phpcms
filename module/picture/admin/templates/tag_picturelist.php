...
<table cellpadding="2" cellspacing="1" class="tableborder">
  ...
  <form name="myform" method="get" action="?" onsubmit="javascript:return doCheck();">
   ...
   <input name="referer" type="hidden" value="<?=$PHP_REFERER?>">
    <tr> 
      <td class="tablerow" width="40%"><b>配置名称</b><font color="red">*</font><br/>只能由字母、数字和下划线组成，例如：new_picture</td>
      <td  class="tablerow">
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>配置说明</b><font color="red">*</font><br/>对标签进行简单描述（可用中文），例如：最新图片</td>
...
选择时频道ID会自动加入到表单中
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>调用图片所属栏目ID</b><br>多个ID之前用半角逗号隔开，0表示不限栏目<br>某些情况下可使用变量<a href="#" onclick="myform.catid.value='$catid'"><font color=red>$catid</font></a>作为参数</td>
      <td  class="tablerow">
<input name="newdata[catid]" type="text" size="15" value="<?=$data[catid]?>" id="catid">
调用子栏目文章<input name='newdata[child]' type='checkbox' value='1' <? if($data[child]==1) { ?>checked<? } ?>>（单个栏目ID有效）
<br>
...
选择时栏目ID会自动加入到表单中
    ...
    <tr> 
      <td class="tablerow"  width="40%"><b>调用图片所属专题ID</b><br>多个ID之前用半角逗号隔开，0表示不限栏目<br>某些情况下可使用变量<a href="#" onclick="myform.specialid.value='$specialid'"><font color=red>$specialid</font></a>作为参数</td>
      <td  class="tablerow">
...
选择时专题ID会自动加入到表单中
    ...
    <tr> 
      ...
      <td  class="tablerow"><input name="newdata[picturenum]" type="text" size="10" value="<?=$data[picturenum]?>"></td>
    </tr>
    <tr> 
      <td class="tablerow" width="40%"><b>图片标题最大字符数</b></td>
      <td  class="tablerow"><input name="newdata[titlelen]" type="text" size="10" value="<?=$data[titlelen]?>"></td>
    </tr>
    <tr> 
      <td class="tablerow" width="40%"><b>图片介绍最大字符数</b></td>
      <td  class="tablerow"><input name="newdata[descriptionlen]" type="text" size="10" value="<?=$data[descriptionlen]?>"></td>
    </tr>
    <tr> 
      <td class="tablerow" width="40%"><b>是否为推荐图片</b></td>
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>多少天以内的图片</b></td>
...
<option value='1' <? if($data[ordertype]==1) { ?>selected<? } ?>>按图片ID降序</option>
<option value='2' <? if($data[ordertype]==2) { ?>selected<? } ?>>按图片ID升序</option>
...
<option value='4' <? if($data[datetype]==4) { ?>selected<? } ?>>格式：<?=date('Y.m.d',$timestamp)?></option>
    ...
    <tr> 
      ...
      <td  class="tablerow"><input type="radio" name="newdata[showcatname]" value="1" <? if($data[showcatname]) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="newdata[showcatname]" value="0" <? if(!$data[showcatname]) { ?>checked<? } ?>>否</td>
    </tr>
    <tr> 
      <td class="tablerow" width="40%"><b>是否在标题后面显示作者</b></td>
      <td  class="tablerow"><input type="radio" name="newdata[showauthor]" value="1" <? if($data[showauthor]) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="newdata[showauthor]" value="0" <? if(!$data[showauthor]) { ?>checked<? } ?>>否</td>
    </tr>
    <tr> 
      <td class="tablerow" width="40%"><b>是否在标题后面显示浏览次数</b></td>
      <td  class="tablerow"><input type="radio" name="newdata[showhits]" value="1" <? if($data[showhits]) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="newdata[showhits]" value="0" <? if(!$data[showhits]) { ?>checked<? } ?>>否</td>
    </tr>
    <tr> 
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>每行显示图片标题列数</b></td>
      <td  class="tablerow">
...
<option value='10' <? if($data[cols]==10) { ?>selected<? } ?>>10列</option>
    ...
    <tr> 
      ...
      <td  class="tablerow">
<?=$showtpl?>
&nbsp;&nbsp;<input type="button" name="edittpl" value="修改选中的模板" onclick="window.location='?dir=plugin&file=template&action=tpledit&templatename=<?=$dtemplate?>&fn='+myform.tpl.value+'.htm&referer=<?=urlencode($PHP_URL)?>'">
      </td>
    </tr>
    <tr> 
