...
<table cellpadding="2" cellspacing="1" class="tableborder">
  ...
  <form name="myform" method="post" action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>">
   <input name="setting[importtime]" type="hidden" <?php if(isset($setting['importtime'])){ ?> value="<?=$setting['importtime']?>" <?php } ?>>
    ...
    <tr> 
      <td class="tablerowhighlight">phpcms 会员字段</td>
      <td class="tablerowhighlight">原系统会员字段</td>
...
{
	if($k == 'groupid' && !$setting[$k]['func']) $setting[$k]['func'] = 'get_groupid';
?>
    <tr> 
...
?>
    <tr> 
	<th colspan=4>会员组对应关系</th>
    </tr>
    <tr>
      <td width='300' class="tablerow" ><b>默认导入到会员组</b></td>
      <td  colspan=3 class="tablerow"><?=$groupid_select?></td>
    </tr>
    <tr> 
      <td class="tablerowhighlight">phpcms 会员组</td>
      <td colspan=3 class="tablerowhighlight">原系统会员组 ID</td>
    </tr>
<?php 
...
?>
    <tr>
      <td width='300' class="tablerow" ><?=$g['groupname']?></td>
      <td  colspan=3 class="tablerow"><input name="setting[groupids][<?=$k?>]" type="text" size="15" <?php if(isset($setting['groupids'][$k])){?>value="<?=$setting['groupids'][$k]?>" <?php } ?>> 多个ID请用逗号分隔</td>
    </tr>
<?php
}
?>
    <tr> 
      <th colspan=4>会员数据导入执行设置</th>
    </tr>
	<tr>
        <td class="tablerow"><b>是否检查同名帐号</b><br>如果选“是”，则系统会自动丢弃同名帐号</td>
        <td colspan=3 class="tablerow"> <input type="radio" name='setting[membercheck]' value='1' <? if($setting['membercheck']) { ?>checked<? } ?>> 是&nbsp;&nbsp;&nbsp;&nbsp;<input type=radio name='setting[membercheck]' value='0' <? if(!$setting['membercheck']) { ?>checked<? } ?>> 否 
        </td>
      </tr>
    <tr> 
