...
<tr>
  ...
  <input type="text" name="data[<?=$i?>][thumb]" id="thumb_<?=$i?>" size="15" value="<?=$data[$i]['thumb']?>" title="缩略图"> <input name="thumb_<?=$i?>_aid" id="thumb_<?=$i?>_aid" type="hidden"><input type="button" name="thumb_upimage" value="上传" style="width:40px" onclick="javascript:openwinx('?mod=phpcms&file=upload&uploadtext=thumb_<?=$i?>','upload','450','350')"/>
...
$().ready(function(){
	...
	$('#template_example').load('?mod=phpcms&file=block&action=get_template_example');
});
