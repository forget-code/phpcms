...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage">促销信息首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage&job=manage">回收站</a>  </td>
...
<td><?=$product['hits']?></td>
<td>
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=edit&label=<?=$product['label']?>&productid=<?=$product['productid']?>&catid=<?=$product['catid']?>" title="编辑产品">编辑</a>
</td>
</tr>
<? } ?>
</table>
...
<table width="100%" align="center" cellpadding="2" cellspacing="1">
   <tr>
    ...
    <td>
	<input type='submit' value='清空回收站' onClick="if(confirm('确认批量删除这些促销信息吗？')) document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=truncate'">
	<input type='submit' value='还原促销' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=status&status=3'">
	<input type='submit' value='删除促销' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=delete'">
</td>
  </tr>
</table>
</form>
