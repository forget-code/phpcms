...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=main&channelid=<?=$channelid?>">信息首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage&job=recycle&catid=<?=$catid?>&channelid=<?=$channelid?>">回收站</a> &gt;&gt; <?=$cat_pos?> </td>
...
<td><?=$info['hits']?></td>
<td>
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=delete&channelid=<?=$channelid?>&infoids=<?=$info['infoid']?>&referer=<?=$referer?>">删除</a> |
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=3&channelid=<?=$channelid?>&infoids=<?=$info['infoid']?>&referer=<?=$referer?>">还原</a>
</td>
</tr>
<? } ?>
...
<table width="100%" height="30" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td>
        ...
        <input name='submit2' type='submit' value='删除信息' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=delete&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp;
        <input name='submit3' type='submit' value='还原信息' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=3&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp;</td>
  </tr>
</table>
