...
<body leftmargin='2' topmargin='0' marginwidth='0' marginheight='0'>
    <form name='myform' method='Post' action='?mod=<?=$mod?>&file=<?=$file?>&channelid=<?=$channelid?>&parentdir=<?=$parentdir?>&currentdir=<?=$currentdir?>'>
...
<td align='right'>
<?php if($currentdir){ ?>
<a href='?mod=<?=$mod?>&file=<?=$file?>&channelid=<?=$channelid?>&type=<?=$type?>&parentdir=<?=$backparentdir?>&currentdir=<?=$parentdir?>'>↑返回上级目录</a>
<?php } ?>
</td>
</tr>
</table>
  ...
  <th colspan="4">文件选择</th>
    ...
    <td width='55' align="right" class="tablerowhighlight" >大小&nbsp;&nbsp;</td>
    <td width='180' class="tablerowhighlight" >&nbsp;类型&nbsp;&nbsp;</td>
    <td width='140' class="tablerowhighlight" >上次修改时间&nbsp;&nbsp;</td>
    </tr>
<?php
...
<a href='?mod=<?=$mod?>&file=<?=$file?>&channelid=<?=$channelid?>&type=<?=$type?>&parentdir=<?=$currentdir?>&currentdir=<?=$ldir[path]?>'><?=$ldir[name]?></a></td>
...
?>
<tr onMouseOut="this.style.backgroundColor='#F1F3F5'" onMouseOver="this.style.backgroundColor='#BFDFFF'" bgcolor="#F1F3F5">
<td align='left' height='20'><?php if($type!="thumb"){ ?><img src='<?=PHPCMS_PATH?>images/ext/<?=$lfile[ext]?>.gif' width='24' height='24'><?php } ?><a href='#' onClick="window.returnValue='<?=$lfile[path]?>|<?=$lfile[size]?>|<?=PHPCMS_PATH?>|<?=$lfile[ext]?>.gif';window.close();"><span onmouseover="ShowADPreview('<?=$lfile['preview']?>')" onmouseout="hideTooltip('dHTMLADPreview')">
<?=$lfile[name]?></span></a></td>
 <td width='55' align='right'><?=$lfile[size]?>&nbsp;K</td>
