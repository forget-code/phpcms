...
function setff(catid)
{
	var arr=new Array();
	<?php echo $cats; ?>
	if(document.getElementById("readpoint").value==0)
...
<form action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&channelid=<?=$channelid?>&articleid=<?=$articleid?>&submit=1" method="post" name="myform" onsubmit="return doCheck();">
...
<option value='0' <? if($includepic==0) { ?>selected<? } ?>>[类别]</option>
<option value='1' <? if($includepic==1) { ?>selected<? } ?>>[图文]</option>
<option value='2' <? if($includepic==2) { ?>selected<? } ?>>[组图]</option>
<option value='3' <? if($includepic==3) { ?>selected<? } ?>>[推荐]</option>
<option value='4' <? if($includepic==4) { ?>selected<? } ?>>[注意]</option>
</select>
 ...
 <tr> 
      ...
      <td class="tablerow">
	  <input name='thumb' type='text' id='thumb' size='60' value='<?=$thumb?>'>
      <input type="button" value="上传图片" onclick="javascript:openwinx('?mod=phpcms&file=uppic&channelid=<?=$channelid?>&uploadtext=thumb&action=thumb&type=thumb&width=150&height=150','upload','350','350')">
    ...
    <tr> 
      ...
      <td class="tablerow"><input name="keywords" type="text" id="keywords" size="40" value='<?=$keywords?>' title='提示;多个关键词请用半角逗号“,”隔开'>
		<?=$keyword_select?>
    ...
    <tr> 
      ...
      <td class="tablerow"><input name='author' type='text' id='author' value='<?=$author?>' size='40' maxlength='30'>
	  <?=$author_select?>
		<input type="checkbox" name="addauthors" value="1"> 添加至作者列表中
    ...
    <tr> 
      ...
      <td class="tablerow">
		<table width='100%' border='0' cellpadding='0' cellspacing='0'>
		<tr>
		<td><input name='copyfromname' type='text' id='copyfromname' value='<?=$copyfromname?>' size='10' maxlength='50'></td>
		<td class="tablerow">地址
		<input name='copyfromurl' type='text' id='copyfromurl'  value='<?=$copyfromurl?>' size='20' maxlength='200'>
		<?=$copyfrom_select?>
		<input type="checkbox" name="addcopyfroms" value="1"> 添加至来源列表中
...
&nbsp;&nbsp;&nbsp;&nbsp;<strong><font color="#0000FF">注：</font></strong><font color="#0000FF">手动分页符标记为“</font> [next] <font color="#0000FF">”，注意大小写</font>
    ...
    <tr id='paginationtype1' style="display:none"> 
      ...
      <td class="tablerow">
自动分页时的每页大约字符数（包含HTML标记）<strong> <input name='maxcharperpage' type='text' id='maxcharperpage' value='<?=$maxcharperpage?>' size='8' maxlength='8'></strong>
   </td>
    </tr>
    <tr> 
      <td class="tablerow">文章性质</td>
      <td class="tablerow">
<input name='ontop' type='checkbox' id='ontop' value='1' <? if($ontop==1) { ?>checked<? } ?>>固顶文章&nbsp;&nbsp;
<input name='elite' type='checkbox' id='elite' value='1' <? if($elite==1) { ?>checked<? } ?>  disabled>推荐文章&nbsp;&nbsp;
文章评分等级
<select name='stars' id='stars'>
	<option value='5' <? if($stars==5) { ?>selected<? } ?>>★★★★★</option>
	<option value='4' <? if($stars==4) { ?>selected<? } ?>>★★★★</option>
	<option value='3' <? if($stars==3) { ?>selected<? } ?>>★★★</option>
	<option value='2' <? if($stars==2) { ?>selected<? } ?>>★★</option>
	<option value='1' <? if($stars==1) { ?>selected<? } ?>>★</option>
	<option value='0' <? if($stars==0) { ?>selected<? } ?>>无</option>
</select>
   </td>
    </tr>
...
<input name='status' type='radio' value='1' <?if($status==1){?>checked<?}?>> 待审核&nbsp;
<input name='status' type='radio' value='0' <?if($status==0){?>checked<?}?>> 草稿&nbsp;
<input name='status' type='radio' value='2' <?if($status==2){?>checked<?}?>> 退槁&nbsp;
<?}?>
	  </td>
	  </tr>
</tbody>
...
<tbody id='Tabs1' style='display:none'>
  <tr>
    ...
    <tr>
      ...
      <td class="tablerow">&nbsp;<input size=5 name="readpoint" type=text value="<?=$readpoint?>"> 点</td>
    </tr>
</tbody>
...
<tr>
<td width='64'>完整标题</td>
<td class="tablerow"><input name='titleintact' type='text' id='titleintact' size='80' maxlength='255' value='<?=$titleintact?>'></td>
</tr>
<tr>
<td width='64'>副 标 题</td>
<td class="tablerow"><input name='subheading' type='text' id='subheading' size='80' maxlength='255' value='<?=$subheading?>'></td>
</tr>
<tr>
<td class="tablerow">&nbsp;</td>
<td class="tablerow"><input name='showcommentlink' type='checkbox' id='showcommentlink' value='1' <? if($showcommentlink) { ?>checked<? } ?>>显示文章列表时在标题旁显示评论链接</td>
</tr>
</table>
    </td>
    </tr>
	...
	<tr> 
      ...
      <td class="tablerow"><script language=javascript>var dateFrom=new MyCalendar("addtime","<?=$addtime?>","选择,年,月,日,上一年,下一年,上个月,下个月,一,二,三,四,五,六"); dateFrom.display();</script>&nbsp;格式：yyyy-mm-dd</td>
    </tr>
    <tr> 
      <td class="tablerow">转向链接</td>
      <td class="tablerow">
       <input name='linkurl' type='text' id='linkurl' value='<? if($linkurl=='') { ?>http://<? } else { ?><?=$linkurl?><? } ?>' size='50' maxlength='255' <? if($linkurl=='') { ?>disabled<? } ?>><input name='uselinkurl' type='checkbox' id='uselinkurl' value='1' onClick='ruselinkurl();' <? if($linkurl!='') { ?>checked<? } ?>><font color='#FF0000'>转向链接</font>
