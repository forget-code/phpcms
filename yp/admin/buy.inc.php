...
$submenu = array(
	array($LANG['manage_buy'],"?mod=$mod&file=$file&action=manage&job=manage"),
	array($LANG['check_info'],"?mod=$mod&file=$file&action=manage&job=check"),
...
$menu = adminmenu($LANG['manage_buy'],$submenu);
switch($action)
{
	case 'manage':	
		if($job == 'manage')
		...
		$category_jump = trade_select('catid', $LANG['please_choose_category_manage'], $catid, "onchange=\"if(this.value!=''){location='?mod=$mod&file=$file&action=manage&typeid=$typeid&job=$job&catid='+this.value;}\"");
		$condition = '';
		...
		$r = $db->get_one("SELECT COUNT(companyid) AS num FROM ".TABLE_YP_BUY." WHERE $status $condition");
		$number = $r['num'];
		$pages = phppages($number,$page,$pagesize);
		$products = array();
		$result = $db->query("SELECT * FROM ".TABLE_YP_BUY." WHERE $status $condition ORDER BY $order LIMIT $offset,$pagesize");		
...
case 'listorder':
	...
	{
		$db->query("UPDATE ".TABLE_YP_BUY." SET `listorder`='$val' WHERE productid=$key ");
	}
...
case 'status' :
	...
	$db->query("UPDATE ".TABLE_YP_BUY." SET status=$status WHERE productid IN ($productids)");
	if($db->affected_rows()>0)
	{
		...
		{	
			...
			{
				extract($db->get_one("SELECT companyid FROM ".TABLE_YP_BUY." WHERE productid=$aid"));
	...
	break;
case 'truncate':
	$db->query("DELETE FROM ".TABLE_YP_BUY." WHERE status='-1'");
	showmessage($LANG['operation_success'],$forward);
	break;
case 'delete':
	if(empty($productid))
	...
	$db->query("DELETE FROM ".TABLE_YP_BUY." WHERE productid IN ($productids)");
	if($db->affected_rows()>0)
	{
		showmessage($LANG['operation_success'],$forward);
	}
...
case 'edit':
	...
	$field = new field($CONFIG['tablepre'].'yp_buy');
	if($dosubmit)
	{
		$field->check_form();
		if($MOD['enableSecondDomain'])
		{
			...
			$product['linkurl'] = 'http://'.$domainName.'.'.$MOD['secondDomain'].'/buy.php?item-'.$productid.'.html';
		}
		else
		{
			...
			$product['linkurl'] = $MODULE['yp']['linkurl'].'web/buy.php?enterprise-'.$userid.'/item-'.$productid.'.html';
		}
		if($background)
		{
			$backgrounds = explode('|',$background);
		...
		$db->query("UPDATE ".TABLE_YP_BUY." SET $sql WHERE productid='$productid'");
	...
	{
		@extract($db->get_one("SELECT * FROM ".TABLE_YP_BUY." WHERE productid='$productid'"),EXTR_OVERWRITE);		
	...
	case 'stats':
		...
		if(is_array($list))
		{
			...
			foreach($list as $catid => $category)
			{
				...
				for($i=-1; $i<4; $i++)
				{
					$r = $db->get_one("select count(productid) as num from ".TABLE_YP_BUY." where status=$i and catid=$catid $sql ");
					$$status[$i] = $r['num'];
				}
