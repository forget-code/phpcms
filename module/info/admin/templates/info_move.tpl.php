...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=main&channelid=<?=$channelid?>">信息首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=move&channelid=<?=$channelid?>">批量移动信息</a> &gt;&gt;</td>
	</td>
  </tr>
</table>
...
<table cellpadding="2" cellspacing="1" class="tableborder">
  <tr>
    <th colspan=3>批量移动信息</th>
  </tr>
<form method="post" name="myform" action="?mod=info&file=info&action=move&channelid=<?=$channelid?>&dosubmit=1">
<input type="hidden" name="referer" value="<?=$referer?>">


<tr>
<td class="tablerow" valign="top"  width="45%">
<input type="radio" name="fromtype" value="1" <? if($infoids){?>checked<?}?> id="fromtype_1" onclick="if(this.checked){$('frombox_1').style.display='';$('frombox_2').style.display='none';}">从指定ID的信息：
<input type="radio" name="fromtype" value="2" <? if(!$infoids){?>checked<?}?> id="fromtype_2" onclick="if(this.checked){$('frombox_1').style.display='none';$('frombox_2').style.display='';}">从指定栏目的信息
<div id="frombox_1" style="display:'';">
<textarea name="infoids" style="height:300px;width:350px;"><?=$infoids?></textarea>
<br/>
<font color="red">Tips:</font>多个信息ID请用','隔开，<font color="red">注意不要换行</font>
</div>
...
function intext(Request)
{
	...
	<tr>
	<td>
	<input type="radio" name="move_mode" value="1" checked onclick="if(this.checked){$('save_original_box').style.display='none';}"> 在目标频道添加信息,链接至本频道原文</td>
	</tr>
	<tr>
	<td>
	<input type="radio" name="move_mode" value="2" onclick="if(this.checked){$('save_original_box').style.display='';}"> 完全移动至目标频道
	<span id="save_original_box" style="display:none"><input type="checkbox" name="save_original" value="1" checked> 保留原文<br/><font color="blue">注意:</font><font color="red">如果选择完全移动，需要在移动成功后在目标频道<b>更新信息地址</b>后重新<b>生成信息</b></font></span>
	</td>
	</tr>
	</table>
</div>
