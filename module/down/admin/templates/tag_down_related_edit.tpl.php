...
?>
	     ...
	     <tr> 
      <td class="tablerow"><b>下载ID</b> 如无特殊需要，请保持默认</td>
      <td  class="tablerow"><input name="tag_config[downid]" type="text" size="30" value="<?=$tag_config['downid']?>"></td>
    </tr> 
    ...
    <tr> 
      <td class="tablerow"><b>调用下载数</b></td>
...
<option value="6" <? if($tag_config['datetype']==6) { ?>selected<? } ?>>格式：<?=date("Y-m-d H:i",$PHP_TIME)?></option>
    ...
    <tr> 
      ...
      <td  class="tablerow">		<?=showtpl($mod,'tag_down_related','tag_config[templateid]',$tag_config['templateid'],'id="templateid"')?>&nbsp;&nbsp;<input type="button" name="edittpl" value="修改选择的模板" onclick="window.location='?mod=phpcms&file=template&action=edit&templateid='+myform.templateid.value+'&module=down&referer=<?=urlencode($PHP_URL)?>'"> [注:只能修改非默认模板]
      </td>
    </tr>
    <tr> 
