...
<form method='post' name='myform' action='?mod=<?=$mod?>&file=<?=$file?>&action=move&channelid=<?=$channelid?>&referer=<?=$referer?>'>
<tr>
<td class="tablerow">
<input type='radio' name='movetype' value='1' <? if($downid){?>checked<?}?>>指定下载ID：<input type='text' name='downid' value='<?=$downid?>' size='40' onclick="this.form.movetype[0].checked=true"><br/>
<input type='radio' name='movetype' value='2' <? if(!$downid){?>checked<?}?>>指定栏目的下载&nbsp;&nbsp;<font color='red'>源栏目可按Ctrl键多选</font><br/>
