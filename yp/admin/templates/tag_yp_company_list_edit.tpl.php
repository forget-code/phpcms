...
<table cellpadding="2" cellspacing="1" class="tableborder">
  ...
  <form name="myform" method="get" action="?">
   ...
   <input name="action" type="hidden" value="edit">
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>标签说明</b><br/>例如：首页最新推荐企业，10篇</td>
	...
	<tr> 
      <td class="tablerow"><b>调用所属行业</b><br><font color="blue">多个ID之前用半角逗号隔开，0或空表示不限栏目</font><br>某些情况下可使用变量<a href="#" onclick="$('tradeid').value='$tradeid'"><font color="red">$tradeid</font></a>作为参数</td>
      <td  class="tablerow">
<input name="tag_config[tradeid]" type="text" size="10"  id="tradeid" value="<?=$tag_config['tradeid']?>">&nbsp;
<?=trade_select('settradeid', '不限', $tag_config['tradeid'], 'onchange="ChangeInput(this,document.myform.tradeid)"')?>
&nbsp;选择时区域ID会自动加入到表单中
</td>
</tr>
<tr> 
      <td class="tablerow" width="40%"><b>是否调用子行业企业</b></td>
      <td  class="tablerow"><input type="radio" name="tag_config[child]" value="1" <? if($tag_config['child']) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="tag_config[child]" value="0" <? if(!$tag_config['child']) { ?>checked<? } ?>>否</td>
    </tr>
    <tr> 
	...
	<tr> 
      <td class="tablerow" width="40%"><b>是否为VIP</b></td>
      <td  class="tablerow"><input type="radio" name="tag_config[vip]" value="1" <? if($tag_config['vip']) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="tag_config[vip]" value="0" <? if(!$tag_config['vip']) { ?>checked<? } ?>>否</td>
    </tr>
    <tr> 
      <td class="tablerow"><b>调用企业数或每页企业数</b></td>
      <td  class="tablerow"><input name="tag_config[number]" type="text" size="10" value="<?=$tag_config['number']?>"></td>
    </tr>
    <tr> 
      <td class="tablerow"><b>企业简介最大字符数</b></td>
      <td  class="tablerow"><input name="tag_config[length]" type="text" size="10" value="<?=$tag_config['length']?>"> 设置为0 则不调用简介</td>
    </tr>
	
	<tr> 
      <td class="tablerow"><b>调用所属经营模式</b></td>
      <td  class="tablerow"><input name="tag_config[pattern]" type="text" size="10" value="<?=$tag_config['pattern']?>" id="pattern"><?=$pattern_select?></td>
    </tr>
    ...
    <tr> 
      <td class="tablerow"><b>多少天以内的企业</b></td>
...
您可以从下拉框中选择
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>企业排序方式</b></td>
      <td  class="tablerow">
<select name='tag_config[ordertype]'>
<option value='0' <? if($tag_config['ordertype']==0) { ?>selected<? } ?>>按企业排序排序</option>
...
<tr> 
      <td class="tablerow" width="40%"><b>是否显示联系人和电话</b></td>
      <td  class="tablerow"><input type="radio" name="tag_config[showcontact]" value="1" <? if($tag_config['showcontact']) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="tag_config[showcontact]" value="0" <? if(!$tag_config['showcontact']) { ?>checked<? } ?>>否</td>
    </tr>
    <tr> 
...
<option value='10' <? if($tag_config['cols']==10) { ?>selected<? } ?>>10列</option>
    ...
    <tr> 
      ...
      <td  class="tablerow">
	<?=showtpl($mod,'tag_company_list','tag_config[templateid]',$tag_config['templateid'],'id="templateid"')?>
