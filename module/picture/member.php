...
$r = $db->get_one("SELECT COUNT(*) as number FROM ".TABLE_PICTURE." WHERE status=3 AND username='$username' AND recycle=0 AND channelid='$channelid' $addquery ");
$number = $r["number"];
$cha['itemnumber'] = $number;

$pages=phppages($number,$page,$pagesize);

$result=$db->query("SELECT pictureid,channelid,catid,title,includepic,titlefontcolor,titlefonttype,hits,username,addtime,editor,edittime,ontop,elite,stars FROM ".TABLE_PICTURE." WHERE status=3 AND username='$username' AND recycle=0 AND channelid=$channelid $addquery ORDER BY $dordertype LIMIT $offset,$pagesize ");
if($db->num_rows($result)>0)
{
	while($r=$db->fetch_array($result))
	{
		$p->set_catid($r[catid]);
		$r[itemurl] = $p->get_itemurl($r[pictureid],$r[addtime]);
...
include template('picture','member');
?>