...
?>
	...
	<tr>
      <td class="tablerow"><b>是否只调用图片影片</b></td>
      <td  class="tablerow"><input type="radio" name="tag_config[onlythumb]" value="1" <? if($tag_config['onlythumb']) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="tag_config[onlythumb]" value="0" <? if(!$tag_config['onlythumb']) { ?>checked<? } ?>>否 无小缩略图的影片不调用</td>
    </tr>
    <tr> 
...
<option value='10' <? if($tag_config['cols']==10) { ?>selected<? } ?>>10列</option>
  ...
  <td  class="tablerow"><input name="tag_config[username]" id="username" type="text" size="15" value="<?=$tag_config['username']?>"/> 可调用单个会员的文章</td>
    ...
    <tr> 
      ...
      <td  class="tablerow">
	<?=showtpl($mod,'tag_movie_thumb','tag_config[templateid]',$tag_config['templateid'],'id="templateid"')?>
