...
<form method="post" name="myform" action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&template=<?=$template?>&save=1" onSubmit="return docheck()">
<table cellpadding="0" cellspacing="1" class="table_form">
  <caption>修改模板</caption>
	...
	<tr>
		...
		<td><font color="blue">./templates/<?=$project?>/<?=$module?>/<?=$filename?></font> (上次修改时间：<?=$filemtime?>) <font color="red">*</font></td>
	</tr>
	<tr>
		<th><strong>模板名称</strong></th>
		<td>
        	<input type="text" name="templatename" value="<?=$templatename?>" size=25> 可以是中文
...
<div class="tag_menu">
	<ul>
	  <li><a href="#" id="menu_tab0" class="selected" onClick="ShowTab(0)">插入标签</a></li>
	  <li><a href="#" id="menu_tab1" onClick="ShowTab(1)">插入变量</a></li>
     </ul>
</div>
<table cellspacing="1" cellpadding="0" class="table_list">
    <tr>
    	<td style="">
        	<span id="tab0">
				<?=form::select($modname, 'selectmodule', 'selectmodule', 'phpcms', 1, '', 'style="width:150px; margin:5px 0;"')?><br />
 ...
 <input type="button" name="template_save" id="template_save" value="恢复数据">
  <div id="toggle_tpl" style="display:none; z-index:1000; width:300px; border:1px solid #99d3fb; height:300px; position:absolute; background:#FFFFFF;">
  <p style="text-align:center;"><?php
  		for($i=1; $i<=10; $i++)
		{
			$bold = $i==1 ? "font-weight: bold;" : '';
			$str .= "<a href='javascript:get_template($i)' class='hour' style='border:#cccccc solid 1px;margin:2px;padding-left:4px;padding-right:4px;$bold' title='第$i页 '>$i</a>";	
		}
		echo $str;
  ?></p>
  <ul id="template_box" class="clear" style="padding:0; margin:0; padding:0 4px;"></ul>
  </div>
  </td>
  </tr>
  </table>
...
function get_fields(val)
{
	...
	$('#selectmodule').change(function(){
		var select = '';
		$('#var').hide();
		$.get('?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>', {action:'showtags', module:$('#selectmodule').val()}, function(data){
			if(data == 'null')
			{
				$('#tag').hide();
				return false;
	...
	$('#template_save').click(function (){
		...
		$('#toggle_tpl').slideToggle('fast');
		$.getJSON("?mod=phpcms&file=template&action=gettemplate", {filename:filename, page:1}, function (data) {
			if(data != null)
			{
				var str = '<li></li>';
				$.each(data, function(i, n){
					if (n.id)
					{
						str += '<li style="border-bottom:dashed #99D3FB 1px; line-height:24px;" class="clear"><span class="float_r" style="width:100px;">'+n.username+'</span><span class="float_l" style="width:180px; padding-left:8px;"><a href="javascript:get_template_data('+n.id+')">'+n.created_time+'</a></span></li>';
					}
				});
				$('#template_box').html(str);
			}
			else
			{
				$('#template_box').html('<li>暂无数据</li>');
			}
		});
	});
	...
	{
		var filename = "<?=$filename?>";
		$.getJSON("?mod=phpcms&file=template&action=gettemplate", {filename:filename, page:page}, function (data) {
			if(data != null)
			{
				var str = '<li></li>';
				$.each(data, function(i, n){
	...
	{
		$.post("?mod=phpcms&file=template&action=gettemplatedata", {id:id}, function (data){
			if(data != '')
			{
				$('#txt_main').text(data);
			}
