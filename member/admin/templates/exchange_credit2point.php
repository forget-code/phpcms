...
<table cellpadding="2" cellspacing="1" class="tableborder">
  ...
  <form name="myform" method="post" action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&save=1" onSubmit='return CheckForm();'>
     <tr> 
      ...
      <td class="tablerow" >1分可兑换<font color="red"><?=$_PHPCMS['credit2point']?></font>点<input type="hidden" name="money2point" value="<?=$_PHPCMS['money2point']?>"><input type="hidden" name="type" value="积分换点数"></td>
    </tr>
     <tr> 
     ...
     <tr> 
      <td class="tablerow" >积分</td>
      <td class="tablerow" ><input type="text" name="credit" value="<?=$credit?>" size=20> 分</td>
    </tr>
     <tr> 
      <td class="tablerow" >可兑换点数</td>
      <td class="tablerow" ><input type="text" name="point" size=20 onclick="myform.point.value=Math.floor(myform.credit.value*<?=$_PHPCMS['credit2point']?>)" readonly> <input type="button" name="submit" value=" 计算点数 " onclick="myform.point.value=Math.floor(myform.credit.value*<?=$_PHPCMS['credit2point']?>)"></td>
    </tr>
   <tr> 
