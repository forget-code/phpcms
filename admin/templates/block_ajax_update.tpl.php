...
<tr>
  <td width="*">
  <input type="text" name="data[<?=$i?>][title]" id="title_<?=$i?>" d="<?=$i?>" value="<?=htmlspecialchars(stripslashes($data[$i]['title']))?>" style="width:100%"><br />
  <input type="text" name="data[<?=$i?>][url]" id="url_<?=$i?>" value="<?=$data[$i]['url']?>" style="width:100%"><br />
  </td>
  <td width="150" class="align_l">
  <input type="text" name="data[<?=$i?>][thumb]" id="thumb_<?=$i?>" size="15" value="<?=$data[$i]['thumb']?>"> <input name="thumb_<?=$i?>_aid" id="thumb_<?=$i?>_aid" type="hidden"><input type="button" name="thumb_upimage" value="上传" style="width:40px" onclick="javascript:openwinx('?mod=phpcms&file=upload&uploadtext=thumb_<?=$i?>','upload','450','350')"/>
  <input type="text" name="data[<?=$i?>][date]" id="date_<?=$i?>" value="<?=$data[$i]['date']?>" size="10">
  </td>
...
<?php } ?>
<div class="button_box" style="padding-left:10px">
<input type="button" name="dosubmit" value="保存碎片" onclick="save_block('<?=$blockid?>', FCKeditorAPI.GetInstance('data').GetData());">&nbsp;&nbsp;
<input type="button" name="block_preview" value="预览碎片" onclick="parent.set_html('<?=$blockid?>', FCKeditorAPI.GetInstance('data').GetData());">&nbsp;&nbsp;
...
function save_block(blockid, blockdata)
{
	$.post('?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&dosubmit=1&blockid='+blockid, {data: blockdata}, function(){
