...
class area_channel
{
	...
	function area_channel($channelid, $areaid = 0)
	{
		global $AREA,$CHA;
		...
		if(!isset($AREA))
		{
			$AREA = cache_read('areas_'.$this->channelid.'.php');
		}
		$this->areas = $AREA;
	...
	function __destruct()
	{
		if($this->updatecache) cache_areas($this->channelid);
	}
	...
	function add($area, $setting = array())
	{
		...
		$db->query("INSERT INTO ".TABLE_INFO_AREA." ($sql1) VALUES($sql2)");
		...
		if($area['parentid'])
		{
			...
			foreach($parentids as $parentid)
			{
				if($parentid)
				{
					...
					$db->query("UPDATE ".TABLE_INFO_AREA." SET child=1,arrchildid='$arrchildid' WHERE areaid='$parentid'");
				}
			}
		}
		else
		...
		$db->query("UPDATE ".TABLE_INFO_AREA." SET arrchildid='$this->areaid',listorder=$this->areaid,arrparentid='$arrparentid' WHERE areaid=$this->areaid");
	...
	function edit($area, $setting = array())
	{
		...
		$db->query("UPDATE ".TABLE_INFO_AREA." SET $sql WHERE areaid=$this->areaid");
	...
	function delete()
	{
		...
		$db->query("DELETE FROM ".channel_table($this->module, $this->channelid)." WHERE areaid IN ($arrchildid)");
		
		$db->query("DELETE FROM ".TABLE_INFO_AREA." WHERE areaid IN ($arrchildid)");
		...
		if($arrparentid)
		{
			...
			foreach($arrparentids as $id)
			{
			    ...
			    $db->query("UPDATE ".TABLE_INFO_AREA." SET arrchildid='$arrchildid' $child WHERE areaid='$id'");
			}
		}
        $this->areaid = 0;
	}
	...
	function listorder($listorder)
	{
		...
		{
			$value = intval($value);
			$db->query("UPDATE ".TABLE_INFO_AREA." SET listorder=$value WHERE areaid=$areaid");
		}
		return TRUE;
	}
	...
	function disable($value)
	{
		...
		$db->query("UPDATE ".TABLE_INFO_AREA." SET disabled='$value' WHERE areaid=$this->areaid");
	...
	function get_list()
	{
		global $db;
		$areas = array();
		$result = $db->query("SELECT areaid FROM ".TABLE_INFO_AREA." WHERE channelid='$this->channelid' ORDER BY listorder");
	...
	function get_info($areaid = 0)
	{
		...
		$data = $db->get_one("SELECT * FROM ".TABLE_INFO_AREA." WHERE areaid=$this->areaid");
		if($data['setting'])
		{
    ...
    function update_linkurl($areaid)
    {
		...
		$db->query("UPDATE ".TABLE_INFO_AREA." SET linkurl='$linkurl' WHERE areaid=$areaid");
        return TRUE;
    }
	...
	function repair()
	{
		...
		if(is_array($this->areas))
		{
			foreach($this->areas as $areaid => $area)
			{
		        ...
		        $db->query("UPDATE ".TABLE_INFO_AREA." SET arrparentid='$arrparentid',arrchildid='$arrchildid',child='$child' WHERE areaid=$areaid");
	...
	function setting($setting)
	{
		...
		$db->query("UPDATE ".TABLE_INFO_AREA." SET setting='$setting' WHERE areaid=$this->areaid");
		return TRUE;
	}
	...
	function move($parentid, $oldparentid)
	{
		...
		if($child)
		{
			foreach($arrchildids as $cid)
			{
				...
				$db->query("UPDATE ".TABLE_INFO_AREA." SET arrparentid='$newarrparentid' WHERE areaid='$cid'");
			}
		}
		...
		if($parentid)
		{
			...
			foreach($arrparentids as $pid)
			{
				...
				$db->query("UPDATE ".TABLE_INFO_AREA." SET arrchildid='$newarrchildid',child=1 WHERE areaid=$pid");
			}
		}
		else
		{
		...
		$db->query("UPDATE ".TABLE_INFO_AREA." SET arrparentid='$arrparentid' WHERE areaid=$this->areaid");
		...
		if($oldparentid)
		{
			foreach($oldarrparentids as $pid)
			{
				...
				$db->query("UPDATE ".TABLE_INFO_AREA." SET arrchildid='$oldarrchildid' $child WHERE areaid=$pid");
			}
		}
		return TRUE;
	}
