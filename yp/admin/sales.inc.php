...
$submenu = array(
	array($LANG['manage_sales'],"?mod=$mod&file=$file&action=manage&job=manage"),
...
case 'listorder':
	...
	{
		$db->query("UPDATE ".TABLE_YP_SALES." SET `listorder`='$val' WHERE productid=$key ");
	}
...
case 'status' :
	...
	$db->query("UPDATE ".TABLE_YP_SALES." SET status=$status WHERE productid IN ($productids)");
	if($db->affected_rows()>0)
	{
		...
		{	
			...
			{
				extract($db->get_one("SELECT companyid FROM ".TABLE_YP_SALES." WHERE productid=$aid"));
...
case 'delete':
	...
	$productids = is_array($productid) ? implode(',',$productid) : intval($productid);
...
case 'edit':
	...
	if($dosubmit)
	{
		...
		{
			extract($db->get_one("SELECT companyname AS pagename,sitedomain AS domainName,templateid AS defaultTplType,banner,background,introduce,menu FROM ".TABLE_MEMBER_COMPANY." WHERE companyid='$companyid'"));
			$product['linkurl'] = 'http://'.$domainName.'.'.$MOD['secondDomain'].'/sales.php?item-'.$productid.'.html';
		}
		else
		{
			...
			$product['linkurl'] = $MODULE['yp']['linkurl'].'web/sales.php?enterprise-'.$userid.'/item-'.$productid.'.html';
		}
		if($background)
		{
			$backgrounds = explode('|',$background);
	...
	case 'stats':
		...
		if(is_array($list))
		{
			...
			foreach($list as $catid => $category)
			{
				...
				for($i=-1; $i<4; $i++)
				{
					$r = $db->get_one("select count(productid) as num from ".TABLE_YP_SALES." where status=$i and catid=$catid $sql ");
					$$status[$i] = $r['num'];
				}
