...
<input name="module" type="hidden" value="phpcms">
...
<table cellpadding="0" cellspacing="1" class="table_form">
	...
	<tr> 
      ...
      <td><input type="text" name="tag_config[module]" id="themodule" size="15" value="<?=$tag_config['module']?>" onBlur="$('#category').load('?mod=phpcms&file=tag&action=ajax_category&module='+this.value);"> <?=form::select_module('select_module', '', '请选择', '', 'onchange="myform.themodule.value=this.value;$(\'#category\').load(\'?mod=phpcms&file=tag&action=ajax_category&module=\'+this.value);"')?></td>
    </tr>
	<tr> 
    ...
    <tr> 
      ...
      <td><?=form::select_template('phpcms', 'tag_config[template]', 'template', $template, '', 'tag_category')?>&nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="location.href='?mod=phpcms&file=template&action=edit&template='+ $('#template').val()+'&module=<?=$mod?>&forward=<?=urlencode(URL)?>'"></td>
    </tr>
    <tr> 
