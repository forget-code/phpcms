...
case 'main':
      ...
      $realpath=realpath("./");
/*
      $result=$db->query("select lastlogintime,logintimes from $table_member where username='$_SESSION[phpcms_username]'");
      $r=$db->fetch_array($result);
      $lastlogintime = $r[lastlogintime] ? date("Y-m-d H:i:s",$r[lastlogintime]) : '';
      $logintimes = $r[logintimes];

      $query = $db->query("SELECT count(*) as num FROM $table_member WHERE groupid!=3");
      $r = $db->fetch_array($query);
      $totalmember = $r[num];
      $query = $db->query("SELECT count(*) as num FROM $table_member WHERE groupid=3");
      $r = $db->fetch_array($query);
      $passmember = $r[num];
      $query = $db->query("SELECT count(*) as num FROM $table_article WHERE passed=1");
      $r = $db->fetch_array($query);
      $totalarticle = $r[num];
      $query = $db->query("SELECT count(*) as num FROM $table_article WHERE passed=0");
      $r = $db->fetch_array($query);
      $passarticle = $r[num];

      $query = $db->query("SELECT count(*) as num FROM $table_comment WHERE passed=1");
      $r = $db->fetch_array($query);
      $totalcomment = $r[num];
      $query = $db->query("SELECT count(*) as num FROM $table_comment WHERE passed=0");
      $r = $db->fetch_array($query);
      $passcomment = $r[num];
      $query = $db->query("SELECT count(*) as num FROM $table_guestbook WHERE passed=1");
      $r = $db->fetch_array($query);
      $totalreply = $r[num];
      $query = $db->query("SELECT count(*) as num FROM $table_guestbook WHERE passed=0");
      $r = $db->fetch_array($query);
      $passreply = $r[num];
*/
