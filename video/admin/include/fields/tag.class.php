...
class model_tag
{
	...
	{
		global $MODEL;
		$content_table_fields = $this->db->get_fields($this->table);
		...
		foreach($this->fields as $field=>$v)
		{
			...
			{
				if($wheresql[$field][0] == DB_PRE.'video_position')
				{
		...
		if($array_diff)
		{
			...
			$tablename = "`".DB_PRE."video` a, `".DB_PRE."video_data` b";
			$whereunion = 'a.vid=b.vid AND a.status=99 ';
			...
			if(is_array($where_array_posid))
			{
				...
				$whereunion .= ' AND a.vid=p.'.$where_array_posid[1].' AND '.$where_array_posid[2];
			}
			if(is_array($where_array_keyword))
			{
				$tablename .= ", `$where_array_keyword[0]` k";
				$whereunion .= ' AND a.vid=k.'.$where_array_keyword[1].' AND '.$where_array_keyword[2];
			}
		}
		else
		{	
			if(is_array($where_array_posid) || is_array($where_array_keyword))
			{
				...
				if(is_array($where_array_posid))
				{
					$tablename = "`".DB_PRE."video` a, `$where_array_posid[0]` p";
					$whereunion = 'a.vid=p.'.$where_array_posid[1].' AND '.$where_array_posid[2].' AND a.status=99 ';
				}
				if(is_array($where_array_keyword))
				{
					if(empty($tablename))
					{
						$tablename .= "`".DB_PRE."video` a, `$where_array_keyword[0]` k";
						$whereunion = 'a.vid=k.'.$where_array_keyword[1].' AND '.$where_array_keyword[2].' AND a.status=99 ';
					}
					else
					{
						$tablename .= ", `$where_array_keyword[0]` k";
