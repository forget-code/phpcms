...
选择时频道ID会自动加入到表单中
    ...
    <tr> 
      <td class="tablerow"  width="40%"><b>调用下载所属栏目ID</b><br>多个ID之前用半角逗号隔开，0表示不限栏目<br>某些情况下可使用变量<a href="#" onclick="myform.catid.value='$catid'"><font color=red>$catid</font></a>作为参数</td>
      <td  class="tablerow">
<input name="newdata[catid]" type="text" size="20" value="<?=$data[catid]?>" id="catid">&nbsp;&nbsp;&nbsp;&nbsp;
<input name='newdata[child]' type='checkbox' value='1' <? if($data[child]==1) { ?>checked<? } ?>>调用子栏目下载（单个栏目ID有效）
<br>
...
选择时栏目ID会自动加入到表单中
    ...
    <tr> 
      <td class="tablerow"  width="40%"><b>调用下载所属专题ID</b><br>多个ID之前用半角逗号隔开，0表示不限栏目<br>某些情况下可使用变量<a href="#" onclick="myform.specialid.value='$specialid'"><font color=red>$specialid</font></a>作为参数</td>
      <td  class="tablerow">
...
选择时专题ID会自动加入到表单中
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>显示下载数</b></td>
      <td  class="tablerow"><input name="newdata[downnum]" type="text" size="10" value="<?=$data[downnum]?>"></td>
    </tr>
    <tr> 
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>是否为推荐下载</b></td>
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>多少天以内的下载</b></td>
...
<option value='1' <? if($data[ordertype]==1) { ?>selected<? } ?>>按下载ID降序</option>
<option value='2' <? if($data[ordertype]==2) { ?>selected<? } ?>>按下载ID升序</option>
