...
<table width="455" cellpadding="2" cellspacing="2" class="tableborder">
  <tr bgColor='#F1F1F1'  height="26">
    <td colspan="8"><font color="#0000FF">操作：</font> <input type="button" onClick="javascript:if(confirm('确认改变审核状态吗？'))  location='?mod=<?=$mod?>&file=<?=$file?>&action=changestatus&job=isverify&odr_id=<?=$odr_id?>&forward=<?=urlencode($PHP_URL)?>'" value="改变审核状态"> 
	<input type="button" onClick="javascript:if(confirm('确认改变付款状态吗？'))  location='?mod=<?=$mod?>&file=<?=$file?>&action=changestatus&job=ispay&odr_id=<?=$odr_id?>&forward=<?=urlencode($PHP_URL)?>'" value="改变付款状态"> 
	<input type="button" onClick="javascript:if(confirm('确认改变发货状态吗？'))  location='?mod=<?=$mod?>&file=<?=$file?>&action=changestatus&job=isship&odr_id=<?=$odr_id?>&forward=<?=urlencode($PHP_URL)?>'" value="改变发货状态">
  </td>
  </tr>
</table>
<table cellpadding="0" cellspacing="0" border="0" width="100%" height="5">
...
<tr align="right" bgColor='#F1F1F1'  height="26">
  <td colspan="8">总计： <strong>￥<?=$total_mount?></strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
</tr>
<tr align="right" bgColor='#F1F1F1'  height="26">
  <td colspan="8">用户折扣： <strong><?=$discount?></strong>&nbsp;&nbsp;总价：￥<strong><?=$total_mount*$discount?></strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
</tr>
</table>
<table width="100%" height="25" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td align="center"><input type="button" value="打印订单" onclick="window.open('?mod=<?=$mod?>&file=<?=$file?>&action=view&odr_id=<?=$odr_id?>&isprint=1');">&nbsp;<input type="button" name="submits" value="返回" onClick="history.back(-1);">
 </td>
  </tr>
</table>
</body>
