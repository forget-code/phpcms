...
switch($action) 
{
	case '':
		if($data == '') exit;
		if(DB_CHARSET != 'utf-8') $data = iconv('utf-8', DB_CHARSET, $data);
	...
	case 'get':
		...
		$data = array();
		$result = $db->query("SELECT `created_time`,`id` FROM ".DB_PRE."editor_data WHERE userid=$_userid AND editorid='$editorid' AND $where_time ORDER BY id DESC");
	...
	case 'get_data':
		...
		if(DB_CHARSET != 'utf-8') $data = iconv(DB_CHARSET, 'utf-8', $r['data']);
		echo $data;
		break;
