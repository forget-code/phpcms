...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage">企业新闻首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage&job=manage">管理新闻</a>  </td>
...
<td><input name='listorder[<?=$article['articleid']?>]' type='input' value='<?=$article['listorder']?>' size="3" /></td>
...
<?php if($article['status']==5) { ?> <font color="blue">荐</font><?php } ?>
</td>

<td><a href="<?=$article['domain']?>" target="_blank" alt="作者：<font color='#ff0000'><?=$article['username']?></font>&nbsp;&nbsp;添加时间：<font color='#ff0000'><?=$article['addtime']?></font>&#10;&nbsp;&nbsp;编辑：<?=$article['editor']?>&nbsp;&nbsp;编辑时间：<?=$article['edittime']?> &#10;&nbsp;&nbsp;审核：<font color='#006600'><?=$article['checker']?></font>&nbsp;&nbsp;审核时间：<font color='#006600'><?=$article['checktime']?></font>"><?=$article['companyname']?></a></td>
<td><?=$article['hits']?></td>
<td>
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=edit&articleid=<?=$article['articleid']?>&companyid=<?=$article['companyid']?>" title="编辑新闻">编辑</a>
</td>
</tr>
<? } ?>
</table>
...
<table width="100%" align="center" cellpadding="2" cellspacing="1" >
   <tr>
	...
	<input type='submit' value='更新地址' onClick="document.myform.action='?mod=<?=$mod?>&file=updateurls&job=<?=$file?>&action=showhtml'">
	<input type='submit' value='推荐新闻' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=status&status=5'">
	<input type='submit' value='取消推荐' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=status&status=3'">
...
<form method="get" name="search" action="?">
  <tr>
	...
	<option value="0" <?if($ordertype==0){?>selected<?}?>>按新闻ID降序</option>
	<option value="1" <?if($ordertype==1){?>selected<?}?>>按新闻ID升序</option>
