...
switch($action)
{
	case 'manage':
		...
		include admin_tpl('news_manage');
		break;

	case 'edit':
		if($id=='') showmessage('请选择要修改的新闻');
		if($dosubmit)
		{
	...
	case 'listorder':
		if($dosubmit)
		{
			...
			if($result)
			{
				showmessage('操作成功！', "?mod=yp&file=news&action=manage");
			}
		}
		else
		{
			if(!$id) showmessage('你没有选择任何新闻',$forward);
			$ids = implodeids($id);
			$where = "id IN ($ids)";
