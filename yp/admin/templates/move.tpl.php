...
<form method="post" name="myform" action="?mod=yp&file=<?=$file?>&action=move&dosubmit=1">
<tr>
<td valign="top"  width="45%">
<input type="radio" name="fromtype" value="0" <? if($id){?>checked<?}?> id="fromtype_1" onclick="if(this.checked){$('#frombox_1').show();$('#frombox_2').hide();}">从指定ID：
<input type="radio" name="fromtype" value="1" <? if(!$id){?>checked<?}?> id="fromtype_2" onclick="if(this.checked){$('#frombox_1').hide();$('#frombox_2').show();}">从指定栏目
<div id="frombox_1" style="display:<? if(!$id){?>none<?}?>;">
<textarea name="id" style="height:300px;width:350px;"><?=$id?></textarea>
<br/>
<font color="red">Tips:</font>多个问题ID请用','隔开，<font color="red">注意不要换行</font>
</div>
<div id="frombox_2" style="display:<? if($id){?>none<?}?>;">
