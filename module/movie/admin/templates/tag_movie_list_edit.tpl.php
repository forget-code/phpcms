...
?>
	...
	<tr> 
	<td class="tablerow"><b>调用影片所属栏目ID</b><br><font color="blue">多个ID之前用半角逗号隔开，0表示不限栏目</font><br>某些情况下可使用变量<a href="###" onclick="$('catid').value='$catid'"><font color="red">$catid</font></a>作为参数</td>
	<td  class="tablerow">
    ...
    <tr> 
      <td class="tablerow"><b>是否调用子栏目影片</b></td>
	...
	<tr> 
      <td class="tablerow"><b>按字母调用</b></td>
      <td  class="tablerow"><input name="tag_config[letter]" type="text" size="10" value="<?=$tag_config['letter']?>"> 例如：a  如果设置为0则表示不限</td>
    </tr>
    <tr> 
    ...
    <tr> 
      <td class="tablerow"><b>多少天以内的影片</b></td>
...
您可以从下拉框中选择
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>影片排序方式</b></td>
      <td  class="tablerow">
<select name='tag_config[ordertype]'>
<option value='0' <? if($tag_config['ordertype']==0) { ?>selected<? } ?>>按影片排序排序</option>
...
<option value='3' <? if($tag_config['ordertype']==3) { ?>selected<? } ?>>按点播次数降序</option>
<option value='4' <? if($tag_config['ordertype']==4) { ?>selected<? } ?>>按点播次数升序</option>
...
<option value="6" <? if($tag_config['datetype']==6) { ?>selected<? } ?>>格式：<?=date("Y-m-d H:i",$PHP_TIME)?></option>
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>是否在影片标题前显示栏目名称</b></td>
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>是否在标题后面显示影片次数</b></td>
      <td  class="tablerow"><input type="radio" name="tag_config[showmovies]" value="1" <? if($tag_config['showmovies']) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="tag_config[showmovies]" value="0" <? if(!$tag_config['showmovies']) { ?>checked<? } ?>>否</td>
    </tr>
    <tr> 
...
<option value='10' <? if($tag_config['cols']==10) { ?>selected<? } ?>>10列</option>
  ...
  <td  class="tablerow"><input name="tag_config[username]" id="username" type="text" size="15" value="<?=$tag_config['username']?>"/> 可调用单个会员的文章</td>
    ...
    <tr> 
      ...
      <td  class="tablerow">
	<?=showtpl($mod,'tag_movie_list','tag_config[templateid]',$tag_config['templateid'],'id="templateid"')?>
&nbsp;&nbsp;<input type="button" name="edittpl" value="修改选择的模板" onclick="window.location='?mod=phpcms&file=template&action=edit&templateid='+myform.templateid.value+'&module=movie&referer=<?=urlencode($PHP_URL)?>'"> [注:只能修改非默认模板]
      </td>
    </tr>
    <tr> 
