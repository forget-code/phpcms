...
<form name="myform" method="post" action="?mod=<?=$mod?>&file=<?=$file?>&action=add" onsubmit="return CheckForm();" enctype="multipart/form-data">
...
<table cellpadding="2" cellspacing="1" class="tableborder">
  ...
  <td class='tablerow'>
<input name='pdt[pdt_name]' type='text' id='pdt_name' size='40' maxlength='50'/>  <font color="red">*</font>&nbsp;&nbsp;&nbsp;<?=$style_edit?>&nbsp;&nbsp;&nbsp;<input type="button" value="检查同名商品" onclick="Dialog('?mod=<?=$mod?>&file=<?=$file?>&action=checktitle&title='+$('pdt_name').value+'','','300','40','no')" style="width:90px;">
  </td>
  </tr>
    <tr>
      <td class='tablerow'><strong>商品编号</strong><br> 不填将自动生成</td>
      <td class='tablerow'><input name='pdt[pdt_No]' type='text' id='pdt_No' size='40' maxlength='50'/> <input type="button" onclick="GetneratePdtNo();" value="生成编号"></td>
    </tr>
    <tr>
...
<?=$position?>
    ...
    <tr>
      ...
      <td class='tablerow'>
      <input style="width:120px;height:20px;" value=""  id="textUin" name="pdt[pdt_brand]" onclick="value=''">
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" onclick="openwinx('?mod=<?=$mod?>&file=brand&action=manage','品牌管理','720','500')" value="品牌管理" /></td>
      ...
      <td class='tablerow'>
<input name='pdt[price]' type='text' id='pdt_price' size='20' maxlength='50'/> 元<font color="red">*</font>
	  </td>
    </tr>
    	<tr>
      <td class='tablerow'><strong>市场价格</strong></td>
      <td class='tablerow'>
<input name='pdt[marketprice]' type='text' id='marketprice' size='20' maxlength='50'/> 元
	  </td>
	  <tr>
    ...
    <tr>
      ...
      <td class='tablerow'><input type='radio' name='pdt[onsale]' id='onsale' value='1' checked>√&nbsp;&nbsp;<input type='radio' name='pdt[onsale]' id='onsale' value='0'>×
      ...
      <td class='tablerow'>
<input name='pdt[pdt_weight]' type='text' id='pdt_weight' size='20' maxlength='50'/> 千克
	  </td>
    </tr>
    	<tr>
      <td class='tablerow'><strong>计量单位</strong></td>
      <td class='tablerow'>
<input name='pdt[pdt_unit]' type='text' id='pdt_unit' size='20' maxlength='50'/>
	  </td>
    </tr>
    <tr>
      <td class='tablerow'><strong>SEO Keywords（商品关键词）</strong><br>针对搜索引擎设置的关键词</td>
      <td class='tablerow'><textarea name='pdt[pdt_keyword]' cols='60' rows='2' id='pdt_keyword'></textarea></td>
    </tr>
    <tr>
      <td class='tablerow'><strong>商品介绍</td>
      <td class='tablerow'><textarea name='pdt[introduce]' cols='80' rows='8' id='introduce'></textarea><?=editor('introduce','introduce',400,200)?>  <font color="red">*</font></td>
    </tr>	
  </tbody>
  ...
  <tbody id='Tabs1' style='display:none'>
    ...
    <tr>
      <td width='20%' class='tablerow' height='40'>&nbsp;<strong>请先选择商品类型</strong></td>
   ...
   ?>
    ...
    <th colspan=2>生成页面</th>
      ...
      <td class="tablerow"><input type="radio" name="pdt[ishtml]" value="1" <?php if($CATE['ishtml']==1) {?>checked <?php } ?>  onclick="$('htmlrule').style.display='';$('htmlprefix').style.display='';$('htmldir').style.display='';$('phprule').style.display='none';"> 是 <input type="radio" name="pdt[ishtml]" value="0" <?php if($CATE['ishtml']==0) {?>checked <?php } ?> onclick="$('htmlrule').style.display='none';$('htmldir').style.display='none';$('htmlprefix').style.display='none';$('phprule').style.display='';"> 否</td>
    </tr>
		<tr id="htmldir" style="display:<?php if($CATE['ishtml']==0) {?>none<?php }?>"> 
		  <td class="tablerow">html文件生成目录</td>
		  <td class="tablerow"><input type="text" name="pdt[htmldir]" value="<?=$CATE['htmldir']?>" id="htmldir" ></td>
		</tr>
		<tr id="htmlprefix" style="display:<?php if($CATE['ishtml']==0) {?>none<?php }?>"> 
		  <td class="tablerow">html文件名前缀</td>
		  <td class="tablerow"><input type="text" name="pdt[prefix]" id="prefix" value="<?=$CATE['item_prefix']?>"></td>
		</tr>
		<tr id="htmlrule" style="display:<?php if($CATE['ishtml']==0) {?>none<?php }?>"> 
		  <td class="tablerow">url规则（生成html）</td>
		  <td class="tablerow"><?=$html_urlrule?></td>
		</tr>
		<tr id="phprule" style="display:<?php if($CATE['ishtml']==1) {?>none<?php }?>"> 
...
<?php
loadMtirPage('product_type',"?mod=$mod&file=attribute&action=list&pro_id=$relatedtype","请选择商品类型，生成属性列表框<br>",0);
?>
</body>
</html>