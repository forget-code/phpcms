...
<table cellpadding="0" cellspacing="1" class="table_form">
    <caption>添加专题</caption>
	  <tr>
      <th width="15%"><strong>所属类别</strong></th>
      <td>
	  <?=form::select_type($mod, 'info[typeid]', 'typeid', '请选择', $typeid, 'require="true" compare=">0" datatype="compare" msgid="specialmsg" msg="请选择所属类别"')?>&nbsp;<a style="color:red" href="?mod=special&file=type&action=add">添加分类</a><span id="specialmsg"></span>
      </td>
    </tr>
	<tr>
    ...
    <tr>
      ...
      <td><textarea name="info[description]" id="description" cols="60" rows="5"></textarea>  <?=form::editor('description','standard','100%', 300)?></td>
    </tr>
	<tr>
      <th><strong>专题页模板</strong></th>
      <td><?=form::select_template($mod, 'info[template]', 'template', '', 'require="true" datatype="limit" msg="请选择模板" msgid="msgid3"', 'special')?> <font color="red">*</font><span id="msgid3"></span></td>
    </tr>
	<tr>
      <th><strong>专题页文件名</strong></th>
      <td><input type="text" name="info[filename]" id="filename" size="25" regexp="^[a-z0-9_]+$" require="true" datatype="limit|ajax" min="1" max="50" url="?mod=<?=$mod?>&file=<?=$file?>&action=checkname" msg="字符长度范围必须为1到50位|" msgid="msgid4"> <font color="#ff0000">.<?=$PHPCMS['fileext']?></font><span id="msgid4"></span></td>
    </tr>
	<tr>
