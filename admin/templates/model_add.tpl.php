...
<form action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>" method="post" name="myform">
    <caption>添加内容模型</caption>
	<tr> 
      <th width="300"><strong>内容模型名称</strong></th>
      <td><input type="text" name="info[name]" size="30" require="true" datatype="limit" min="1" max="50"  msg="字符长度范围必须为1到50位"> <font color="red">*</font></td>
    </tr>
	<tr> 
      <th><strong>内容模型描述</strong></th>
      <td><textarea name="info[description]" rows="2" cols="20" id="description" class="inputtext" style="height:43px;width:208px;"></textarea></td>
    </tr>
	<tr> 
	...
	<tr> 
      ...
      <td><?=DB_PRE?>c_<input type="text" name="info[tablename]" size="12" regexp="^[a-z0-9_]+$" require="true" datatype="limit|custom" min="1" max="50"  msg="字符长度范围必须为1到50位|只能为数字、和字母，下划线"> <font color="red">*</font></td>
    </tr>
     <tr>
	...
	<tr> 
      ...
      <td><?=form::select_template('phpcms', 'info[template_category]', 'template_category','category','require="true" datatype="limit" msg="请选择栏目页模板"','category')?> <font color="red">*</font></td>
    </tr>
	<tr> 
      <th><strong>列表页模板</strong></th>
      <td><?=form::select_template('phpcms', 'info[template_list]', 'template_list','list','require="true" datatype="limit" msg="请选择列表页模板"','list')?> <font color="red">*</font></td>
    </tr>
	<tr> 
      <th><strong>内容页模板</strong></th>
      <td><?=form::select_template('phpcms', 'info[template_show]', 'template_show','show','require="true" datatype="limit" msg="请选择内容页模板"','show')?> <font color="red">*</font></td>
    </tr>
	<tr> 
	...
	<tr>
	  ...
	  <a href="?mod=phpcms&file=urlrule&action=add&module=phpcms&filename=category&forward=<?=urlencode(URL)?>" style="color:red">点击新建URL规则</a>
	  </th>
      <td><div id="div_category_urlruleid"></div></td>
    </tr>
	<tr>
      <th><strong>内容页URL规则</strong><br />
	  <a href="?mod=phpcms&file=urlrule&action=add&module=phpcms&filename=show&forward=<?=urlencode(URL)?>" style="color:red">点击新建URL规则</a>
      </th>
	...
	<tr> 
      ...
      <td>
	  <input type='radio' name='info[enablesearch]' value='1' checked /> 是&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type='radio' name='info[enablesearch]' value='0' /> 否
	  </td>
    </tr>
	<tr> 
      <th><strong>前台发布信息是否需要审核</strong></th>
      <td>
	  <input type='radio' name='info[ischeck]' value='1' checked/> 是&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type='radio' name='info[ischeck]' value='0' /> 否
	  </td>
    </tr>
	<tr> 
      <th><strong>发布、编辑信息时更新相关信息</strong></th>
      <td>
	  <input type='radio' name='info[isrelated]' value='1' checked> 是&nbsp;&nbsp;&nbsp;&nbsp;
	  <input type='radio' name='info[isrelated]' value='0' /> 否 
    ...
    <tr> 
      <th></th>
      <td>
	  <input type="hidden" name="info[modeltype]" value="0">
