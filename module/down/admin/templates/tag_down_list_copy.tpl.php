...
<option value="6" <? if($tag_config['datetype']==6) { ?>selected<? } ?>>格式：<?=date("Y-m-d H:i",$PHP_TIME)?></option>
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>是否在下载标题前显示栏目名称</b></td>
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>是否在标题后面显示下载次数</b></td>
      <td  class="tablerow"><input type="radio" name="tag_config[showdowns]" value="1" <? if($tag_config['showdowns']) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="tag_config[showdowns]" value="0" <? if(!$tag_config['showdowns']) { ?>checked<? } ?>>否</td>
    </tr>
    <tr> 
...
<option value='10' <? if($tag_config['cols']==10) { ?>selected<? } ?>>10列</option>
  ...
  <td  class="tablerow"><input name="tag_config[username]" id="username" type="text" size="15" value="<?=$tag_config['username']?>"/> 可调用单个会员的文章</td>
    ...
    <tr> 
      ...
      <td  class="tablerow">
	<?=showtpl($mod,'tag_down_list','tag_config[templateid]',$tag_config['templateid'],'id="templateid"')?>
