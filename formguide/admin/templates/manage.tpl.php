...
<input type="hidden" name="file" value="<?=$file?>">
	<table cellpadding="0" cellspacing="1" class="table_list">
		...
		<tr>
			...
			<td><a href="<?=$M[url]?>index.php?formid=<?=$info[formid]?>" target="_blank"><?=$info[name]?> [访问前台]</a><?php if($formguide_admin->rows(DB_PRE.'form_'.$info[tablename])) {?>(<a href="?mod=<?=$mod?>&file=view&action=manage&formid=<?=$info[formid]?>" title="查看信息"><font color="red"><?=$formguide_admin->rows(DB_PRE.'form_'.$info[tablename])?></font></a>)<?php } ?></td>
			<td>
			...
			<td class="align_c" style="width:200px;">
			...
			<a href="#" onClick="javascript:confirmurl('?mod=<?=$mod?>&file=<?=$file?>&action=delete&formid=<?=$info[formid]?>', '确认要删除‘<?=$info['name']?>’吗？')" title="删除">删除</a> | <a href="?mod=<?=$mod?>&file=<?=$file?>&action=stat&formid=<?=$info[formid]?>">统计</a>
			</td>
		</tr>
		<?php
			}
