...
switch($action)
{
	//这里先做简单的删除，完成任务插入后要判断该站点下面是否有任务存在 再执行删除操作
	case 'delete':
	...
	case 'catchsiteinfo':  //搜集用户信息
			...
			while ($row = $db->fetch_row($rows)) {
				$comma = "";
				for($i = 1; $i < $numfields; $i++) { //i从1开始，前面两项分别是id和站点id，在导入时重新生成
	...
	case 'siteout':
		...
		else 
		{
			...
			while ($row = $db->fetch_row($rows)) {
				$comma = "";
				for($i = 1; $i < $numfields; $i++) { //i从1开始，前面两项分别是id，在导入时重新生成
					$tabledump .= $comma."'".new_addslashes($row[$i])."'";
					$comma = ",";
				}
				$tabledump .= ");\n";
	...
	case 'sitein':
		...
		else 
		{
			if(isset($uploadjob))
			{
				...
				$savepath = "/data/temp/";
				dir_create($savepath);
				$upload = new upload($fileArr,'sitein.psite',$savepath,$upfile_type,1,$upfile_size);
				if($upload->up())
