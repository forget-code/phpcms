...
?>
<body>
<form action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&typeid=<?=$typeid?>&module=<?=$module?>" method="post" >
<table class="table_form" cellspacing="1" cellpadding="0">
	<caption> 修改分类 </caption>
	<tr>
      <th width="150"><strong>分类名称</strong></th>
      <td><input type="text" name="info[name]" id="name" value="<?=$name?>" size="30" require="true" datatype="require" msg="请填写类别名称" msgid="namemsg"> <?=form::style('info[style]', $style)?> <font color="red">*</font><span id="namemsg"></span></td>
    </tr>
	<tr>
