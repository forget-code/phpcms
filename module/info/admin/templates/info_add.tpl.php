...
<table cellpadding="2" cellspacing="1" class="tableborder">
  ...
  <tr>
    ...
    <tr> 
      ...
      <td class="tablerow"><?=$type_select?> <input name="info[title]" type="text" id="title" size="44" maxlength="100" class="inputtitle" onBlur="segment_word(this);"> <?=$style_edit?> <input type="button" value="检查同名标题" onclick="Dialog('?mod=<?=$mod?>&file=<?=$file?>&action=checktitle&channelid=<?=$channelid?>&title='+$('title').value+'','','300','40','no')" style="width:90px;"></td>
    </tr>
    <tr> 
      <td class="tablerow">所在地 <font color="#FF0000">*</font></td>
      <td class="tablerow"><?=ajax_area_select('info[areaid]',$channelid);?></td>
    </tr>
     ...
     <tr <?php if(!$MOD['keywords_show']){ ?>style="display:none"<?php } ?>> 
      ...
      <td class="tablerow"><input name="info[keywords]" type="text" id="keywords" size="40" title="提示:多个关键字请用半角逗号“,”或空格隔开" onblur="doKeywords(this);">
		<?=$keywords_select?>
    ...
    <tr> 
	  ...
	  <tr>
	  <td valign="top">
        <textarea name="info[content]" id="content"  cols="100" rows="25"></textarea><?=editor("content", $MOD['editor_mode'],$MOD['editor_width'],$MOD['editor_height'])?>
    </td>
</tr>
</table>
    ...
    <tr> 
      ...
      <td class="tablerow"><input name="info[address]" type="text" id="address" size="40" maxlength="30" value=""></td>
    </tr>

    <tr> 
      <td class="tablerow">联系人</td>
      <td class="tablerow"><input name="info[author]" type="text" id="author" size="40" maxlength="30" value="<?=$_username?>"></td>
    </tr>

    <tr> 
      <td class="tablerow">电话</td>
      <td class="tablerow"><input name="info[telephone]" type="text" id="telephone" size="40"></td>
    </tr>

	    <tr> 
      <td class="tablerow">Email</td>
      <td class="tablerow"><input name="info[email]" type="text" id="email" size="40"></td>
    </tr>
...
<input name="info[status]" type="radio" value="3" <?php if($_grade<4){?>checked<?php }else{ ?>disabled<?php } ?>> 已通过&nbsp;
<input name="info[status]" type="radio" value="1" <?php if($_grade==4){?>checked<?php } ?>> 待审核&nbsp;
<input name="info[status]" type="radio" value="0"> 草稿&nbsp;
    ...
    <tr> 
      ...
      <td class="tablerow"><input type="radio" name="info[ishtml]" value="1" <?php if($CAT['ishtml']==1) {?>checked <?php } ?>  onclick="$('htmlrule').style.display='';$('htmlprefix').style.display='';$('htmldir').style.display='';$('phprule').style.display='none';"> 是 <input type="radio" name="info[ishtml]" value="0" <?php if($CAT['ishtml']==0) {?>checked <?php } ?> onclick="$('htmlrule').style.display='none';$('htmldir').style.display='none';$('htmlprefix').style.display='none';$('phprule').style.display='';"> 否</td>
		...
		<tr id="htmldir" style="display:<?php if($CAT['ishtml']==0) {?>none<?php }?>"> 
		  <td class="tablerow">html信息生成目录</td>
		  <td class="tablerow"><input type="text" name="info[htmldir]" value="<?=$CAT['item_htmldir']?>" id="htmldir" ></td>
		</tr>
		<tr id="htmlprefix" style="display:<?php if($CAT['ishtml']==0) {?>none<?php }?>"> 
		  <td class="tablerow">html信息名前缀</td>
		  <td class="tablerow"><input type="text" name="info[prefix]" id="prefix" value="<?=$CAT['item_prefix']?>"></td>
		</tr>
...
<tbody id="Tabs2" style="display:none">
  <tr>
    ...
    <tr>
      ...
      <td class="tablerow">&nbsp;<input size="5" name="info[readpoint]" type="input" value="<?=$CAT['defaultpoint']?>" /> 点</td>
    </tr>
