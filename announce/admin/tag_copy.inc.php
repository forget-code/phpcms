...
if($dosubmit)
{
	$tagname or showmessage($LANG['tag_name_required'],"goback");
	if($tag->exists($tagname)) showmessage($LANG['tag'] . $tagname . $LANG['already_exist_rename'],'goback');
...
else
{
	...
	include admintpl('tag_announce_list_copy');
}
?>