...
END;
	...
	$sql=$db->query("select pictureid,catid,content,title,elite,addtime from ".TABLE_PICTURE." where elite>0 and status=3 and recycle=0 and channelid=$channelid and catid in($cat[arrchildid]) order by pictureid desc limit 0,$rows");
	while($r=$db->fetch_array($sql)){
		  $p->set_catid($r[catid]);
	      $r[url] = "http://".$PHP_DOMAIN.$p->get_itemurl($r[pictureid],$r[addtime]);
...
END;
    ...
    $result=$db->query("select pictureid,content,catid,title,ontop,elite,addtime from ".TABLE_PICTURE." where elite>0 and status=3 and recycle=0  and channelid=$channelid order by ontop desc,pictureid desc limit 0,$rows");
