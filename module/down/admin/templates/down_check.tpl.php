...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=main&channelid=<?=$channelid?>">下载首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage&job=check&catid=<?=$catid?>&channelid=<?=$channelid?>">审核下载</a> &gt;&gt; <?=$cat_pos?> </td>
...
<td><input name='downids[]' type='checkbox' value='<?=$down['downid']?>'></td>

<td><?=$down['downid']?></td>

<td align="left"><? if(!$catid) { ?>[<a href="<?=$CATEGORY[$down['catid']]['linkurl']?>" target="_blank"><?=$CATEGORY[$down['catid']]['catname']?></a>] <? } ?><a href="?mod=<?=$mod?>&file=<?=$file?>&action=preview&channelid=<?=$channelid?>&downid=<?=$down['downid']?>&referer=<?=$referer?>"><?=$down['title']?></a></td>
...
<td><?=$down['adddate']?></td>
<td>
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=edit&channelid=<?=$channelid?>&downid=<?=$down['downid']?>&catid=<?=$down['catid']?>&referer=<?=$referer?>">编辑</a> |
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=-1&downids=<?=$down['downid']?>&channelid=<?=$channelid?>&referer=<?=$referer?>">删除</a>
</td>
</tr>
<? } ?>
...
<table width="100%" height="30" border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	<input name='submit0' type='submit' value='通过审核' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=3&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp;
		<input name='submit7' type='submit' value='删除下载'		onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=-1&channelid=<?=$channelid?>&referer=<?=$referer?>'">&nbsp;
        <input name='submit8' type='submit' value='移动下载' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=move&movetype=1&channelid=<?=$channelid?>'"></td>
  </tr>
</table>
