...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=main&channelid=<?=$channelid?>">文章首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage&job=check&catid=<?=$catid?>&channelid=<?=$channelid?>">审核文章</a> &gt;&gt; <?=$cat_pos?> </td>
...
<td width="110" class="tablerowhighlight">管理操作</td>
</tr>

<? if(is_array($articles)) foreach($articles AS $article) { ?>
<tr align="center" onmouseout="this.style.backgroundColor='#F1F3F5'" onmouseover="this.style.backgroundColor='#BFDFFF'" bgColor='#F1F3F5'>

<td><input name='articleids[]' type='checkbox' value='<?=$article['articleid']?>'></td>

<td><?=$article['articleid']?></td>

<td align="left"><? if(!$catid) { ?>[<a href="<?=$CATEGORY[$article['catid']]['linkurl']?>" target="_blank"><?=$CATEGORY[$article['catid']]['catname']?></a>] <? } ?><a href="?mod=<?=$mod?>&file=<?=$file?>&action=preview&channelid=<?=$channelid?>&articleid=<?=$article['articleid']?>&referer=<?=$referer?>"><?=$article['title']?></a></td>
...
<td>
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=edit&channelid=<?=$channelid?>&articleid=<?=$article['articleid']?>&catid=<?=$article['catid']?>&referer=<?=$referer?>">编辑</a> |
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=sendback&channelid=<?=$channelid?>&articleid=<?=$article['articleid']?>&catid=<?=$article['catid']?>&referer=<?=$referer?>">退稿</a> |
<a href="?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=-1&articleids=<?=$article['articleid']?>&channelid=<?=$channelid?>&referer=<?=$referer?>">删除</a>
</td>
</tr>
<? } ?>
...
<table width="100%" height="30" border="0" cellpadding="0" cellspacing="0">
  <tr>
	...
	<input name='submit0' type='submit' value='通过审核' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=3&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp;
        <input name='submit2' type='submit' value='批量退稿' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=2&channelid=<?=$channelid?>&referer=<?=$referer?>'" >&nbsp;
		<input name='submit7' type='submit' value='删除文章'		onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=action&job=status&value=-1&channelid=<?=$channelid?>&referer=<?=$referer?>'">&nbsp;
        <input name='submit8' type='submit' value='移动文章' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=move&movetype=1&channelid=<?=$channelid?>'"></td>
  </tr>
</table>
