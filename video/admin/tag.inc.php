...
{
    case 'add':
		$r = $db->get_one("SELECT modelid FROM ".DB_PRE."model WHERE modeltype=9 AND tablename='$type'");

			require CACHE_MODEL_PATH.'video_tag_form.class.php';
			$content_tag_form = new model_tag_form($modelid);
	...
	case 'edit':
		...
		require CACHE_MODEL_PATH.'video_tag_form.class.php';
		$yp_tag_form = new model_tag_form($modelid);
		$forminfos = $yp_tag_form->get($where);
		$fields = $orderfields = array();
		foreach($yp_tag_form->fields as $field=>$v)
		{
			$fields[$field] = $v['name'];
			if($v['isorder'])
			{
	...
	case 'update':
		...
		$tagname = trim($tagname);
			...
			if(!$tag_config['mode'])
			{
				...
				require CACHE_MODEL_PATH.'video_tag.class.php';
				$vp_tag = new model_tag($modelid);
				$tag_config['sql'] = $vp_tag->get($tag_config['selectfields'], $info, $tag_config['orderby']);
			}
