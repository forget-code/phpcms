...
可用中文，不得包含特殊字符 ' &quot; $ { } ( ) \ / ,</span></td>
    <td width="67%" height="25" ><input name="tagname" id="tagname" type="text" size="20" value="<?=$tagname?>"disabled="disabled" />
  ...
  <tr bgcolor="#F1F3F5">
    ...
    <td height="25" >是
      <label>
      <input type="radio" name="tag_config[page_on]" id="page_on" value="1" <?php if ($tag_config[page_on]==1){?> checked="checked" <?php }?> />
      </label>
      否
      <label>
      <input type="radio" name="tag_config[page_on]" id="page_on" value="0" <?php if ($tag_config[page_on]==0){?> checked="checked" <?php }?> />
      </label></td>
  </tr>
  <tr bgcolor="#F1F3F5">
    <td height="25" ><strong>调用文章数或每页文章数</strong></td>
    <td height="25" ><input name="tag_config[list_num]" type="text" id="list_num" value="<?=$tag_config['list_num']?>" size="10" /></td>
  </tr>
  <tr bgcolor="#F1F3F5">
    <td height="25" ><strong>文章标题最大字符数</strong></td>
    <td height="25" ><input name="tag_config[title_text]" type="text" id="title_text" value="<?=$tag_config['title_text']?>" size="10" /></td>
  </tr>
  <tr bgcolor="#F1F3F5">
    <td height="25" ><strong>内容摘要最大字符数</strong></td>
    <td height="25" ><input name="tag_config[text_num]" type="text" id="text_num" value="<?=$tag_config['text_num']?>" size="10" /></td>
  </tr>
  <tr bgcolor="#F1F3F5">
...
【如果选择不限制栏目，请关闭此项！】</label></td>
  ...
  <tr>
      ...
      <td height="25" bgcolor="#F1F3F5"  class="tablerow"><input name="tag_config[digg_channelid]" id="setchannelid" type="text" size="15" value="<?=$tag_config['digg_channelid']?>">
	...
	<input name="tag_config[digg_catid]" type="text" size="15"  id="catid" value="<?=$tag_config['digg_catid']?>">
	<span id="category_select">
	<select name='selectcatid' onChange="ChangeInput(this,document.myform.catid)">
	<option value="0">不限栏目</option>
    </select>
	<? //=$category_select?>
    </span>&nbsp;【注:当使用总列表的时候选择&quot;不限栏目&quot;】 </td>
	</tr>

    <tr bgcolor="#F1F3F5">
      <td height="25" class="tablerow"><b>多少天以内的排行</b></td>
      <td height="25"  class="tablerow"><input name="tag_config[digg_defind]" type="text" size="10" value="<?=$tag_config['digg_defind']?>" id="datenum"> 天&nbsp;&nbsp;&nbsp;&nbsp;
...
您可以从下拉框中选择</td>
  ...
  <tr bgcolor="#F1F3F5">
    ...
    <td height="25" bgcolor="#F1F3F5" >是
      <label>
      <input name="tag_config[comment_on]" type="radio" id="" value="1" <?php if ($tag_config[comment_on]==1){?> checked="checked" <?php }?>/>
      </label>
否
<label>
<input type="radio" name="tag_config[comment_on]" id="" value="0" <?php if ($tag_config[comment_on]==0){?> checked="checked" <?php }?>/>
<span class="tablerow">【注:开启后略微消耗系统资源】 </span></label></td>
  ...
  <tr bgcolor="#F1F3F5">
    ...
    <td height="25" bgcolor="#F1F3F5" >是
      <label>
      <input name="tag_config[hits_on]" type="radio" id="" value="1" <?php if ($tag_config[hits_on]==1){?> checked="checked" <?php }?> />
      </label>
否
<label>
<input type="radio" name="tag_config[hits_on]" id="" value="0" <?php if ($tag_config[hits_on]==0){?> checked="checked" <?php }?> />
<span class="tablerow">【注:开启后略微消耗系统资源】 </span></label></td>
  </tr>
      <tr bgcolor="#F1F3F5">
      <td width="40%" height="25" class="tablerow"><b>此标签调用的模板</b></td>
      <td height="25"  class="tablerow">
<?=showtpl($mod,'tag_digg_list','tag_config[templateid]',$tag_config['templateid'],'id=templateid')?>
&nbsp;&nbsp;
