...
class model
{
	...
	function cache()
	{
		...
		cache_common();
		$fields = array();
		$files = glob(PHPCMS_ROOT.'include/fields/*');
	...
	function cache_class($fields, $classname)
	{
		...
		{
			$r = @file_get_contents(PHPCMS_ROOT.'include/fields/'.$field.'/'.$classname.'.inc.php');
			if($r) $data .= $r;
		}
		$classfile = 'content_'.$classname.'.class.php';
		$classcode = @file_get_contents(PHPCMS_ROOT.'include/fields/'.$classfile);
