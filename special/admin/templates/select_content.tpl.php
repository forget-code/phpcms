...
<form name="myform" method="post" action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&specialid=<?=$specialid?>"  onsubmit="return checkform();">
...
if(is_array($infos)){
	foreach($infos as $info){
		$isexists = $db->get_one("SELECT `specialid` FROM `".DB_PRE."special_content` WHERE `specialid`=$specialid AND `contentid`=".$info['contentid']);
?>
<tr>
...
<td class="align_c">
<?php if($isexists) { ?>
<font color="gray">加入</font><?php }else{ ?><a href="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&specialid=<?=$specialid?>&contentid=<?=$info['contentid']?>&dosubmit=1" title="加入专题">加入</a><?php }?>
|
...
<input type="submit" name="dosubmit" value="加入专题"  />
</div>
