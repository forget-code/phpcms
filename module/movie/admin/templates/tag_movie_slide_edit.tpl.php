...
?>
   ...
   <tr> 
      <td class="tablerow"><b>调用图片格式</b></td>
      <td  class="tablerow"><input type="radio" name="tag_config[thumbtype]" value="1" <?php if($tag_config['thumbtype']){ ?>checked<?php } ?>>大缩略图&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="tag_config[thumbtype]" value="0" <?php if(!$tag_config['thumbtype']){ ?>checked<?php } ?>>小缩略图</td>
    </tr>
...
<option value='23' <?php if($tag_config['effectid']==23) { ?>selected<?php } ?>>随机任意的上述一种效果</option>
  ...
  <td  class="tablerow"><input name="tag_config[username]" id="username" type="text" size="15" value="<?=$tag_config['username']?>"/> 可调用单个会员的文章</td>
    ...
    <tr> 
      ...
      <td  class="tablerow">
	<?=showtpl($mod,'tag_movie_slide','tag_config[templateid]',$tag_config['templateid'],'id="templateid"')?>
