...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$mod?>&action=main&channelid=<?=$channelid?>">图片首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=tohtml&channelid=<?=$channelid?>">发布网页</a> </td>
  </tr>
</table>
...
<tr align="center">
<td width="25%" class="tablerowhighlight">图片起始ID</td>
<td width="25%" class="tablerowhighlight">图片结束ID</td>
<td width="25%" class="tablerowhighlight">每轮生成图片数</td>
...
<input type="hidden" name="action" value="show" id="action">
  <tr align="center">
    ...
    <td class="tablerow"><input name="pernum" type="text" size="15" value="50"></td>
    <td class="tablerow"><input name='submit' type='submit' value='生成图片'>&nbsp;&nbsp;<input type="submit" onclick="$('action').value='url';" value="更新地址" ></td>
  </tr>
</form>
</table>
...
<?=$categorys?>
  <tr>
	...
	<input type="submit" name="submit2" value="生成图片" onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=cat_show&channelid=<?=$channelid?>'">&nbsp;&nbsp;
