...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=main&channelid=<?=$channelid?>">影片首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=move&channelid=<?=$channelid?>">批量移动影片</a> &gt;&gt;</td>
	</td>
  </tr>
</table>
...
<table cellpadding="2" cellspacing="1" class="tableborder">
  <tr>
    <th colspan=3>批量移动影片</th>
  </tr>
<form method="post" name="myform" action="?mod=movie&file=movie&action=move&channelid=<?=$channelid?>&dosubmit=1">
<input type="hidden" name="referer" value="<?=$referer?>">


<tr>
<td class="tablerow" valign="top"  width="45%">
<input type="radio" name="fromtype" value="1" <? if($movieids){?>checked<?}?> id="fromtype_1" onclick="if(this.checked){$('frombox_1').style.display='';$('frombox_2').style.display='none';}">从指定ID的影片：
<input type="radio" name="fromtype" value="2" <? if(!$movieids){?>checked<?}?> id="fromtype_2" onclick="if(this.checked){$('frombox_1').style.display='none';$('frombox_2').style.display='';}">从指定栏目的影片
<div id="frombox_1" style="display:'';">
<textarea name="movieids" style="height:300px;width:350px;"><?=$movieids?></textarea>
<br/>
<font color="red">Tips:</font>多个影片ID请用','隔开，<font color="red">注意不要换行</font>
</div>
...
function intext(Request)
{
	...
	<tr>
	<td>
	<input type="radio" name="move_mode" value="1" checked onclick="if(this.checked){$('save_original_box').style.display='none';}"> 在目标频道添加影片,链接至本频道原文</td>
	</tr>
	<tr>
	<td>
	<input type="radio" name="move_mode" value="2" onclick="if(this.checked){$('save_original_box').style.display='';}"> 完全移动至目标频道
	<span id="save_original_box" style="display:none"><input type="checkbox" name="save_original" value="1" checked> 保留原文<br/><font color="blue">注意:</font><font color="red">如果选择完全移动，需要在移动成功后在目标频道<b>更新影片地址</b>后重新<b>生成影片</b></font></span>
	</td>
	</tr>
	</table>
</div>
