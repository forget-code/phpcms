...
if(is_array($list))
{
	...
	foreach($list as $catid => $category)
	{
		...
		for($i=-1; $i<4; $i++)
		{
			$r = $db->get_one("select count(articleid) as num from ".channel_table('article', $channelid)." where status=$i and catid=$catid $sql ");
			$$status[$i] = $r['num'];
		}
		...
		$categorys[$category['catid']] = array('id'=>$category['catid'],'parentid'=>$category['parentid'],'name'=>$category['catname'],'listorder'=>$category['listorder'],'style'=>$category['style'],'mod'=>$mod,'channelid'=>$channelid,'file'=>$file,'num__1'=>$num__1,'num_0'=>$num_0,'num_1'=>$num_1,'num_2'=>$num_2,'num_3'=>$num_3,'percent__1'=>$percent__1,'percent_0'=>$percent_0,'percent_1'=>$percent_1,'percent_2'=>$percent_2,'percent_3'=>$percent_3);
	}
