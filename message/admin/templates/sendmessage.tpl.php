...
<table cellpadding="0" cellspacing="1" class="table_form">
    ...
    <tr>
        ...
        <td>
        	<input type="radio" name="type" value="group" <?=($type=='group')?'checked':''?> onclick="$('#roleid').hide();$('#groupid').show();"  /> 按用户组&nbsp;&nbsp;
            <input type="radio" name="type" value="role" <?=($type=='role')?'checked':''?>  onclick="$('#groupid').hide();$('#roleid').show();" /> 按角色
    ...
    <tr>
    	...
    	<td>
			<?=form::select($GROUP, $name = 'groupid', $id = 'groupid', $groupid)?>
        	<?=form::select($role_group, $name = 'roleid', $id = 'roleid', $roleid, 'style="display:none"')?>
        </td>
    </tr>
	<tr>
