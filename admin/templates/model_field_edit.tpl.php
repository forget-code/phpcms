...
<form action="?mod=<?=$mod?>&file=<?=$file?>&action=<?=$action?>&modelid=<?=$modelid?>&fieldid=<?=$fieldid?>&formtype=<?=$formtype?>&issystem=<?=$issystem?>" method="post" name="myform">
...
?>
	...
	<tr>
      ...
      <td><input type="radio" name="info[isfulltext]" value="1" <?=($isfulltext ? 'checked' : '')?> /> 是 <input type="radio" name="info[isfulltext]" value="0"  <?=($isfulltext ? '' : 'checked')?> /> 否</td>
    </tr>
	<tr>
