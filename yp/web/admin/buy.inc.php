...
$submenu = array(
	array("<font color='#ff0000'>".$LANG['publish_buy_info']."</font>","?file=$file&action=add"),
	array($LANG['manage_buy'],"?file=$file&action=manage&job=manage&label=4"),
...
switch($action)
{
	case 'add':
		if($dosubmit)
		{
			...
			$sql = "INSERT INTO ".TABLE_YP_BUY." ($sql1) VALUES($sql2)";
			$result = $db->query($sql);
			...
			{
				$linkurl = $mydomain.'/buy.php?item-'.$productid.'.html';
			}
			else
			{
				$linkurl = $MODULE['yp']['linkurl'].'web/buy.php?enterprise-'.$domainName.'/item-'.$productid.'.html';
				
			}
			$db->query("UPDATE ".TABLE_YP_BUY." SET linkurl='$linkurl' WHERE productid=$productid ");
			$product['introduce'] = stripslashes($product['introduce']);
			createhtml('buy',PHPCMS_ROOT.'/yp/web');
	...
	case 'status':
		...
		$db->query("UPDATE ".TABLE_YP_BUY." SET status=$status WHERE productid IN ($productids) AND username='$_username'");
