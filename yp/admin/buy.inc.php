...
switch($action)
{
	case 'manage':
		...
		include admin_tpl('buy_manage');
		break;

	case 'edit':
		if($id=='') showmessage('请选择要修改的商机');
		if($dosubmit)
		{
	...
	case 'listorder':
		if($dosubmit)
		{
			...
			if($result)
			{
				showmessage('操作成功！', "?mod=yp&file=buy&action=manage");
			}
		}
		else
		{
			if(!$id) showmessage('你没有选择任何商机',$forward);
			$ids = implodeids($id);
			$where = "id IN ($ids)";
