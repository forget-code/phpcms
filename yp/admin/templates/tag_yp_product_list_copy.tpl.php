...
<tr> 
      <td class="tablerow" width="40%"><b>是否在标题前显示产品属性</b></td>
      <td  class="tablerow"><input type="radio" name="tag_config[showlabel]" value="1" <? if($tag_config['showlabel']) { ?>checked<? } ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="tag_config[showlabel]" value="0" <? if(!$tag_config['showlabel']) { ?>checked<? } ?>>否</td>
</tr>
    <tr> 
...
<option value="6" <? if($tag_config['datetype']==6) { ?>selected<? } ?>>格式：<?=date("Y-m-d H:i",$PHP_TIME)?></option>
    ...
    <tr> 
      <td class="tablerow" width="40%"><b>是否在产品标题前显示栏目名称</b></td>
