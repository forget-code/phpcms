...
class category_module
{
	...
	function category_module($module, $catid = 0)
	{
		global $CATEGORY;
		...
		if(!isset($CATEGORY))
		{
			$CATEGORY = cache_read('categorys_'.$this->module.'.php');
		}
	...
	function __destruct()
	{
		if($this->updatecache) cache_categorys($this->module);
	}
	...
	function get_list()
	{
		global $db;
		$categorys = array();
		$result = $db->query("SELECT catid FROM ".TABLE_CATEGORY." WHERE module='$this->module' ORDER BY listorder");
    ...
    function update_linkurl($catid)
    {
		...
		$db->query("UPDATE ".TABLE_CATEGORY." SET linkurl='$linkurl' WHERE catid=$catid");
        return TRUE;
    }
