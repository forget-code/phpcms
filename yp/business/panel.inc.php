...
@extract($r);
@extract($db->get_one("SELECT count(gid) AS new_guestbook_number FROM `".DB_PRE."yp_guestbook` WHERE userid='$_userid' AND status=0"));
@extract($db->get_one("SELECT count(messageid) AS new_message_number FROM `".DB_PRE."message` WHERE send_to_id='$_userid' AND status=0"));
