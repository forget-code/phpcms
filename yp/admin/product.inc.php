...
switch($action)
{
	case 'manage':
		...
		include admin_tpl('product_manage');
		break;

	case 'edit':
		if($id=='') showmessage('请选择要修改的产品');
		if($dosubmit)
		{
		...
		else
		{
			...
			include admin_tpl('product_edit');
		}
		break;
	...
	case 'listorder':
		if($dosubmit)
		{
			...
			if($result)
			{
				showmessage('操作成功！', "?mod=yp&file=product&action=manage");
			}
		}
		else
		{
			if(!$id) showmessage('你没有选择任何产品',$forward);
			$ids = implodeids($id);
			$where = "id IN ($ids)";
	...
	case 'move':
		if($dosubmit)
		{
			...
			showmessage($LANG['operation_success'], "?mod=yp&file=$file&action=manage");
		}
		else
		{
