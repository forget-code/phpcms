...
<input name="forward" type="hidden" value="?mod=video&file=tag&action=manage&page=<?=$page?>">
...
<tbody>
    ...
    <tr> 
      ...
      <td><?=form::select_template($module, 'tag_config[template]', 'template', 'tag_', '', 'tag_')?>&nbsp;<input type="button" value="修改选中模板" title="修改选中模板" onClick="location.href='?mod=phpcms&file=template&action=edit&template='+ $('#template').val()+'&module=<?=$mod?>&forward=<?=urlencode(URL)?>'"></td>
    </tr>
    <tr> 
