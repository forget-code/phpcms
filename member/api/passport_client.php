...
parse_str(StrCode($userdb,'DECODE'), $member);
$member['cookietime'] = $member['cktime'] ? $member['cktime'] - $PHP_TIME : 0;
...
if($action == 'login')
{
	...
	elseif($member['url'])
	{
		...
		$member_encode = str_replace('=', '', StrCode($member_encode));
		$verify = md5('login'.$member_encode.$forward.$db_hash);
		header("location:$jumpurl/passport_client.php?action=login&userdb=".rawurlencode($member_encode)."&forward=".rawurlencode($forward)."&verify=".rawurlencode($verify));
		exit;
	}
...
elseif($action == 'quit')
{
	...
	elseif($member['url'])
	{
		...
		$member_encode = str_replace('=', '', StrCode($member_encode));
		$verify = md5('quit'.$member_encode.$forward.$db_hash);
		header("location:$jumpurl/passport_client.php?action=quit&userdb=".rawurlencode($member_encode)."&forward=".rawurlencode($forward)."&verify=".rawurlencode($verify));
		exit;
	}
	header('location:'.$forward);
}
