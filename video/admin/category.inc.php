...
switch($action)
{
	...
	case 'updatecache':
		...
		foreach($CATEGORY AS $k=>$c)
		{
			$catid = $c['catid'];
			$url = caturl();
			$r = $db->get_one("SELECT count(vid) AS num FROM ".DB_PRE."video WHERE catid=$catid");
			$db->query("UPDATE ".DB_PRE."category SET items=$r[num],url='$url' WHERE catid=$catid");
		}
		cache_common();
		cache_category();
		showmessage($LANG['category_cache_update_success'], '?mod=video&file=category&action=manage');
		break;
