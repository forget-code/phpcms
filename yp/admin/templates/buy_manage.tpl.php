...
<table width="100%" height="25" border="0" cellpadding="3" cellspacing="0" class="tableborder">
  <tr>
    <td class="tablerow"> <img src="<?=PHPCMS_PATH?>admin/skin/images/pos.gif" align="absmiddle" alt="当前位置" > 当前位置：<a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage">求购信息首页</a> &gt;&gt; <a href="?mod=<?=$mod?>&file=<?=$file?>&action=manage&job=manage">求购管理</a>  </td>
...
<table width="100%" align="center" cellpadding="2" cellspacing="1">
   <tr>
	...
	<input type='submit' value='更新地址' onClick="document.myform.action='?mod=<?=$mod?>&file=updateurls&action=buy'">
	<input type='submit' value='推荐求购' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=status&status=5'">
	<input type='submit' value='取消推荐' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=status&status=3'">
	<input type='submit' value='删除求购' onClick="document.myform.action='?mod=<?=$mod?>&file=<?=$file?>&action=status&status=-1'">
</td>
  </tr>
</table>
</form>
